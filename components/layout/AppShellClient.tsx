...
const PRIMARY_NAV = [
  { id: 'home',    href: '/home',         label: 'Home',        icon: 'M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z M9 22V12h6v10' },
...
export default function AppShellClient({ user, conversations, children }) {
  ...
  useEffect(() => {
    fetch('/api/alerts').then(r => r.json()).then(d => setAlertCount(Array.isArray(d) ? d.filter(a => a.is_active).length : 0)).catch(() => {})
    ...
    <div style={{display:'flex',minHeight:'100vh',background:'var(--bg)',fontFamily:'var(--font-dm,DM Sans)'}}>
      ...
      <aside className="desktop-sidebar" style={{width:240,flexShrink:0,background:'var(--sf)',borderRight:'1px solid var(--b)',display:'flex',flexDirection:'column',height:'100vh',position:'fixed',left:0,top:0,bottom:0,zIndex:99,transform:sidebarOpen?'translateX(0)':'translateX(-100%)',transition:'transform 280ms cubic-bezier(.16,1,.3,1)'}} id="sidebar">
        ...
        <div style={{padding:'8px 8px 4px',flexShrink:0}}>
          ...
          <button onClick={() => setMoreOpen(v => !v)} style={{display:'flex',alignItems:'center',gap:9,padding:'9px 10px',borderRadius:10,border:'none',background:'transparent',color:'var(--tx3)',fontSize:13,fontWeight:500,cursor:'pointer',fontFamily:'inherit',width:'100%',marginTop:2}}>
        ...
        <div onClick={() => setConvOpen(v => !v)} style={{padding:'5px 10px',flexShrink:0,display:'flex',alignItems:'center',justifyContent:'space-between',cursor:'pointer',userSelect:'none'}}
        ...
        <div style={{padding:'8px 8px 4px',borderTop:'1px solid var(--b)',flexShrink:0}}>
          <Link href="/billing" style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:'9px',borderRadius:10,background:'#6366F1',color:'#fff',textDecoration:'none',fontSize:13,fontWeight:600,fontFamily:'var(--font-sora)',transition:'opacity 150ms'}}
        ...
        <div style={{padding:'4px 8px 10px',flexShrink:0}}>
          <div onClick={() => setProfileOpen(true)} style={{display:'flex',alignItems:'center',gap:8,padding:'7px 9px',borderRadius:9,cursor:'pointer',transition:'background 150ms'}}
            onMouseEnter={e => e.currentTarget.style.background = 'var(--ev)'}
            onMouseLeave={e => e.currentTarget.style.background = 'transparent'}>
      ...
      {sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{position:'fixed',inset:0,zIndex:98,background:'rgba(0,0,0,.4)'}} className="mobile-only"></div>}
