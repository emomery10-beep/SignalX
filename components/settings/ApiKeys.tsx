...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button
      onClick={copy}
      style={{ fontSize: 9, fontWeight: 600, color: copied ? '#16a34a' : 'var(--acc)', background: 'transparent', border: `1px solid ${copied ? 'rgba(34,197,94,.3)' : 'rgba(208,138,89,.3)'}`, borderRadius: 'var(--r-md)', padding: '4px 10px', cursor: 'pointer', fontFamily: 'inherit', minHeight: 28, transition: 'all 150ms' }}
    >
...
export default function ApiKeys() {
  ...
  const inputStyle: React.CSSProperties = {
    width: '100%', padding: '9px 11px', fontSize: 11,
    ...
    <div>
      ...
      {justCreated && (
        <div style={{ background: 'rgba(34,197,94,.06)', border: '1px solid rgba(34,197,94,.2)', borderRadius: 'var(--r-lg)', padding: '14px 16px', marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 9, color: 'var(--tx3)', marginBottom: 8 }}>{justCreated.name}</div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: 'var(--ev)', borderRadius: 'var(--r-md)', padding: '8px 11px' }}>
            <code style={{ flex: 1, fontSize: 10, color: 'var(--tx)', fontFamily: 'var(--font-mono)', wordBreak: 'break-all' }}>
              {justCreated.key}
            </code>
            <CopyButton text={justCreated.key}/>
          </div>
          <button onClick={() => setJustCreated(null)} style={{ marginTop: 10, fontSize: 10, color: 'var(--tx3)', background: 'transparent', border: 'none', cursor: 'pointer', fontFamily: 'inherit', minHeight: 'unset' }}>
      ...
      {/* Error */}
      {error && (
        <div style={{ fontSize: 10, color: '#dc2626', background: 'rgba(220,38,38,.05)', border: '1px solid rgba(220,38,38,.15)', borderRadius: 'var(--r-md)', padding: '9px 12px', marginBottom: 12 }}>
          {error}
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ padding: '24px', textAlign: 'center', background: 'var(--ev)', borderRadius: 'var(--r-lg)', marginBottom: 12 }}>
          ...
          <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--tx)', marginBottom: 4 }}>{tc('settings_apikeys.noKeysTitle')}</div>
          <div style={{ fontSize: 11, color: 'var(--tx3)' }}>{tc('settings_apikeys.noKeysSubtitle')}</div>
        </div>
      ) : (
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, marginBottom: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 3, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx)' }}>{k.name}</span>
                      <span style={{ fontSize: 9, padding: '1px 7px', borderRadius: 'var(--rf)', background: plan.bg, color: plan.colour, fontWeight: 600 }}>
                        {plan.label}
                      </span>
                      <span style={{ fontSize: 9, padding: '1px 7px', borderRadius: 'var(--rf)', background: 'var(--ev)', color: 'var(--tx3)', fontWeight: 500 }}>
                        {k.mode}
                      </span>
                      {!k.is_active && (
                        <span style={{ fontSize: 9, padding: '1px 7px', borderRadius: 'var(--rf)', background: 'rgba(220,38,38,.08)', color: '#dc2626', fontWeight: 600 }}>
                          {tc('settings_apikeys.disabled')}
                        </span>
                      )}
                    </div>
                    <code style={{ fontSize: 9, color: 'var(--tx3)', fontFamily: 'var(--font-mono)' }}>{k.key}</code>
                  </div>
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    <button
                      onClick={() => toggleKey(k.id, k.is_active)}
                      style={{ fontSize: 9, color: k.is_active ? '#d97706' : '#16a34a', background: 'transparent', border: `1px solid ${k.is_active ? 'rgba(245,158,11,.3)' : 'rgba(34,197,94,.3)'}`, borderRadius: 'var(--r-md)', padding: '4px 9px', cursor: 'pointer', fontFamily: 'inherit', minHeight: 28 }}
                    >
                      {k.is_active ? tc('settings_apikeys.disable') : tc('settings_apikeys.enable')}
                    </button>
                    <button
                      onClick={() => deleteKey(k.id)}
                      style={{ fontSize: 9, color: '#dc2626', background: 'transparent', border: '1px solid rgba(220,38,38,.25)', borderRadius: 'var(--r-md)', padding: '4px 9px', cursor: 'pointer', fontFamily: 'inherit', minHeight: 28 }}
                    >
      ...
      {showForm ? (
        <div style={{ background: 'var(--ev)', border: '1px solid var(--b2)', borderRadius: 'var(--r-lg)', padding: '14px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx)', marginBottom: 12 }}>{tc('settings_apikeys.newApiKeyTitle')}</div>
          <div style={{ marginBottom: 10 }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', marginBottom: 4 }}>{tc('settings_apikeys.keyNameLabel')}</div>
            <input
              style={inputStyle}
              value={newKeyName}
          ...
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 10, color: 'var(--tx3)', marginBottom: 6 }}>{tc('settings_apikeys.modeLabel')}</div>
            <div style={{ display: 'flex', gap: 8 }}>
              {(['generic', 'account'] as const).map(m => (
                <button
                  ...
                  style={{
                    flex: 1, padding: '9px', fontSize: 10, fontWeight: 500,
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button
              ...
              style={{ flex: 1, padding: '10px', fontSize: 11, fontWeight: 600, background: 'var(--acc)', color: '#fff', border: 'none', borderRadius: 'var(--r-md)', cursor: creating ? 'wait' : 'pointer', fontFamily: 'inherit', opacity: creating ? .7 : 1, boxShadow: '0 2px 8px rgba(208,138,89,.25)' }}
            >
              {creating ? tc('settings_apikeys.creating') : tc('settings_apikeys.createKey')}
            </button>
            <button
              onClick={() => { setShowForm(false); setNewKeyName(''); setError('') }}
              style={{ padding: '10px 16px', fontSize: 11, color: 'var(--tx3)', background: 'transparent', border: '1px solid var(--b2)', borderRadius: 'var(--r-md)', cursor: 'pointer', fontFamily: 'inherit' }}
            >
        ...
        <button
          ...
          style={{ width: '100%', padding: '10px', fontSize: 11, fontWeight: 600, background: 'transparent', border: `1px dashed rgba(208,138,89,.4)`, borderRadius: 'var(--r-lg)', color: 'var(--acc)', cursor: 'pointer', fontFamily: 'inherit', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}
        >
      ...
      <div style={{ marginTop: 14, padding: '10px 13px', background: 'var(--ev)', borderRadius: 'var(--r-md)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
        ...
        <a
          href="/developers"
          target="_blank"
          style={{ fontSize: 10, fontWeight: 600, color: 'var(--acc)', textDecoration: 'none', whiteSpace: 'nowrap' }}
        >
