      ...
      <span key={li}>
        {parts.map((part, pi) =>
          part.startsWith('**') && part.endsWith('**')
            ? <strong key={pi} style={{ color: 'var(--tx)', fontWeight: 600 }}>{part.slice(2, -2)}</strong>
...
export default function ResultBlock({ result, question, onFollowUp, geo, cfoMode }: Props) {
  ...
  const handleExportReport = () => {
    ...
    a.href = url
    a.download = `askbiz-report-${new Date().toISOString().slice(0,10)}.txt`
    a.click()
  ...
  return (
    <div style={{ fontSize: 11, lineHeight: 1.65 }}>
      ...
      {/* 3. KPI cards — always shown when present */}
      {hasKpis && <KpiCardBlock cards={result.kpi_cards!}/>}

      {/* 4. Chart — always shown when present */}
      {hasChart && (
        <MiniLineChart
