    ...
    <div style={CARD}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--tx)' }}>{tc('cfo_recovered.title')}</span>
        </div>
      ...
      <div style={{ padding: '14px 18px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: RED, fontVariantNumeric: 'tabular-nums' }}>{fmt(stats.total_amount, sym)}</div>
            <div style={{ fontSize: 9, color: 'var(--tx3)', marginTop: 2 }}>{tc('cfo_recovered.failedPayments', { n: stats.total_failed })}</div>
          </div>
          <div>
            <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', marginBottom: 4 }}>{tc('cfo_recovered.recovered')}</div>
            <div style={{ fontSize: 16, fontWeight: 700, color: GREEN, fontVariantNumeric: 'tabular-nums' }}>{fmt(stats.recovered_amount, sym)}</div>
            <div style={{ fontSize: 9, color: 'var(--tx3)', marginTop: 2 }}>{tc('cfo_recovered.payments', { n: stats.recovered_count })}</div>
          </div>
          <div>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: stats.recovery_rate >= 50 ? GREEN : stats.recovery_rate >= 25 ? AMBER : RED, fontVariantNumeric: 'tabular-nums' }}>
              {stats.recovery_rate.toFixed(0)}%
            </div>
            <div style={{ fontSize: 9, color: 'var(--tx3)', marginTop: 2 }}>{tc('cfo_recovered.pending', { n: stats.pending_retry })}</div>
          </div>
        </div>
      </div>
    </div>
