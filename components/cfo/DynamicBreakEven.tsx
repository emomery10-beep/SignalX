    ...
    <div style={CARD}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--b)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--tx)' }}>Break-Even Analysis</span>
          <span style={{ fontSize: 10, fontWeight: 600, padding: '2px 8px', borderRadius: 4,
            background: isProfitable ? 'rgba(34,197,94,.1)' : 'rgba(245,158,11,.1)',
            color: isProfitable ? GREEN : AMBER }}>
            {isProfitable ? 'Above break-even' : `${fmt(revenueToGo, sym)} to go`}
          </span>
        </div>
        <button onClick={() => onAsk(`My break-even revenue is ${fmt(breakEvenRevenue, sym)} per period. Current revenue: ${fmt(revenue, sym)} (${rawProgressPct.toFixed(0)}% of break-even). Fixed costs: ${fmt(fixed_costs, sym)}, gross margin: ${gross_margin_pct}%. Safety margin: ${safetyMarginPct.toFixed(1)}%. How can I lower my break-even point?`)}
      ...
      <div style={{ padding: '16px 18px' }}>
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx)' }}>Revenue Progress</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: 'var(--tx3)', marginTop: 4 }}>
            <span>{sym}0</span>
            <span style={{ fontWeight: 600 }}>Break-even: {fmt(breakEvenRevenue, sym)}</span>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: 'var(--b)', borderRadius: 8, overflow: 'hidden', marginBottom: 14 }}>
          <div style={{ padding: '10px 8px', background: 'var(--sf)', textAlign: 'center' }}>
            <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', marginBottom: 3 }}>Break-Even</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', fontVariantNumeric: 'tabular-nums' }}>{fmt(breakEvenRevenue, sym)}</div>
          </div>
          <div style={{ padding: '10px 8px', background: 'var(--sf)', textAlign: 'center' }}>
            <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', marginBottom: 3 }}>Safety Margin</div>
          ...
          <div style={{ padding: '10px 8px', background: 'var(--sf)', textAlign: 'center' }}>
            <div style={{ fontSize: 9, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', marginBottom: 3 }}>Days to B/E</div>
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)', fontVariantNumeric: 'tabular-nums' }}>
              {daysToBreakEven != null ? `${daysToBreakEven}d` : '—'}
            </div>
          </div>
        </div>

        {/* Breakdown */}
        <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>How It&apos;s Calculated</div>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12, marginBottom: 14 }}>
          <tbody>
            <tr style={{ borderBottom: '1px solid var(--b)' }}>
              <td style={{ padding: '6px 0', color: 'var(--tx)' }}>Fixed Costs (period)</td>
              <td style={{ padding: '6px 0', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{fmt(fixed_costs, sym)}</td>
            </tr>
            <tr style={{ borderBottom: '1px solid var(--b)' }}>
              <td style={{ padding: '6px 0', color: 'var(--tx)' }}>Gross Margin %</td>
              <td style={{ padding: '6px 0', textAlign: 'right', fontVariantNumeric: 'tabular-nums', fontWeight: 600 }}>{gross_margin_pct.toFixed(1)}%</td>
            </tr>
            <tr style={{ borderBottom: '1px solid var(--b)' }}>
              <td style={{ padding: '6px 0', color: 'var(--tx)', fontWeight: 600 }}>Break-Even = Fixed Costs / GM%</td>
        ...
        <div style={{ padding: 12, borderRadius: 8, border: '1px solid var(--b)', background: 'var(--ev, #f9f9f8)', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--tx)' }}>Orders to Break Even</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ fontSize: 10, color: 'var(--tx3)' }}>Avg order:</span>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-around', textAlign: 'center' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 9, color: 'var(--tx3)' }}>to go</div>
            </div>
          </div>
        </div>
        ...
        {monthlyBE.length > 1 && (
          <>
            <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Monthly Break-Even Trend</div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 9, color: 'var(--tx3)' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: GREEN, opacity: 0.7 }} /> Hit break-even</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: AMBER, opacity: 0.7 }} /> Below break-even</span>
            </div>
          </>
        )}
      </div>
