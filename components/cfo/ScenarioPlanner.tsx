...
const PRESETS: { label: string; icon: string; scenario: Partial<Scenario> }[] = [
  { label: 'Lose 20% revenue', icon: '📉', scenario: { revenueChange: -20 } },
  { label: 'Hire new employee', icon: '👤', scenario: { newMonthlyExpense: 35000 } },
  { label: 'Supplier +15% costs', icon: '📦', scenario: { cogsChange: 15 } },
  { label: 'Open 2nd location', icon: '🏪', scenario: { newMonthlyExpense: 80000 } },
  { label: 'Raise prices 10%', icon: '💰', scenario: { priceChange: 10, revenueChange: 5 } },
  { label: 'Seasonal dip 30%', icon: '🌧️', scenario: { revenueChange: -30 } },
]
...
export default function ScenarioPlanner({ baseRevenue, baseCogs, baseFixed, cashBalance, currencySymbol: sym }: Props) {
  ...
  if (cashBalance > 0 && dailyBurn < 0) {
    ...
    runwayLabel = `${runwayMonths} months`
  } else if (dailyBurn >= 0) {
    runwayLabel = 'Cash positive'
  } else {
    runwayLabel = 'Set cash balance'
  }
  ...
  return (
    <div style={{ borderRadius: 14, border: '1px solid var(--b)', background: 'var(--sf)', overflow: 'hidden' }}>
      <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--b)' }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--tx)' }}>Scenario Planner</div>
        <div style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 2 }}>What happens if...? Adjust the sliders to model different scenarios.</div>
      </div>

      {/* Presets */}
      <div style={{ padding: '12px 18px', borderBottom: '1px solid var(--b)' }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>Quick Scenarios</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 0 }}>
        ...
        <div style={{ padding: '16px 18px', borderRight: '1px solid var(--b)' }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Adjust Assumptions</div>

          <SliderControl label="Revenue change" value={s.revenueChange} min={-50} max={50} step={5} unit="%" onChange={v => update('revenueChange', v)} />
          <SliderControl label="COGS change" value={s.cogsChange} min={-30} max={30} step={5} unit="%" onChange={v => update('cogsChange', v)} />
          <SliderControl label="New monthly expense" value={s.newMonthlyExpense} min={0} max={200000} step={5000} unit={sym} onChange={v => update('newMonthlyExpense', v)} />
          <SliderControl label="One-time expense" value={s.oneTimeExpense} min={0} max={1000000} step={10000} unit={sym} onChange={v => update('oneTimeExpense', v)} />
        </div>

        {/* Right: Results */}
        <div style={{ padding: '16px 18px' }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: 'var(--tx3)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Projected Monthly</div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            <ResultRow label="Revenue" base={baseRevenue} projected={projRevenue} sym={sym} />
            <ResultRow label="COGS" base={baseCogs} projected={projCogs} sym={sym} negative />
            <ResultRow label="Fixed costs" base={baseFixed} projected={projFixed} sym={sym} negative />
            <div style={{ borderTop: '2px solid var(--b)', paddingTop: 8 }}>
              <ResultRow label="Net Profit" base={baseNet} projected={projNet} sym={sym} bold />
            </div>
            <ResultRow label="Net Margin" base={baseMargin} projected={projMargin} sym="" unit="%" isPercent />

            <div style={{ borderTop: '1px solid var(--b)', paddingTop: 8, marginTop: 4 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                <span style={{ fontSize: 11, color: 'var(--tx3)' }}>Cash Runway</span>
                <span style={{
              ...
              {baseRunway != null && runwayMonths != null && (
                <div style={{ fontSize: 10, color: 'var(--tx3)' }}>
                  Currently: {baseRunway} months → Scenario: {runwayMonths} months
                  <span style={{ marginLeft: 4, color: runwayMonths > baseRunway ? '#22C55E' : '#EF4444', fontWeight: 600 }}>
                    ({runwayMonths > baseRunway ? '+' : ''}{(runwayMonths - baseRunway).toFixed(1)} months)
                  </span>
                </div>
              )}
            </div>
            ...
            <div style={{
              ...
              {revenueGap >= 0 ? (
                <div style={{ fontSize: 11, color: '#22C55E', fontWeight: 500 }}>
                  {fmt(revenueGap, sym)}/month above break-even
                </div>
              ) : (
                <div style={{ fontSize: 11, color: '#EF4444', fontWeight: 500 }}>
                  {fmt(Math.abs(revenueGap), sym)}/month below break-even — need to increase revenue or cut costs
                </div>
              )}
            </div>
          </div>
