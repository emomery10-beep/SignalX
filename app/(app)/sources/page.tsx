...
export default function SourcesPage() {
  ...
  return (
    <FeatureGate feature="sources" featureName="Data Sources" planNeeded="growth" planId={planLoading ? 'free' : (planId || 'free')}>
        ...
        <div className="page-shell-body">
          ...
          {pageLoading ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 14 }}>
              {[1,2,3,4,5,6].map(i => <div key={i} style={{ height: 160, borderRadius: 16, background: 'var(--ev)', animation: 'shimmer 1.4s infinite' }}/>)}
            </div>
          ) : (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 14 }}>
              {SOURCES.map(src => {
