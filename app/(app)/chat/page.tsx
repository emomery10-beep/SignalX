  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div ref={chatRef} style={{ flex: 1, overflowY: 'auto', padding: '16px 0' }}>
        <div style={{ maxWidth: 680, margin: '0 auto', padding: '0 18px', display: 'flex', flexDirection: 'column', gap: 2 }}>
          ...
          {isEmpty && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, textAlign: 'center', padding: 28 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--tx2)', lineHeight: 1.65, maxWidth: 360, marginBottom: 26 }}>
                {geo ? `Showing prices in ${geo.currencySymbol} · ${geo.country}` : 'Upload a file or connect a data source, then ask a business question in plain English.'}
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, maxWidth: 460, width: '100%' }}>
                {suggestions.map((q, i) => (
          ...
          {messages.map(msg => (
            <div key={msg.id} style={{ display: 'flex', gap: 9, padding: '2px 0', flexDirection: msg.role === 'user' ? 'row-reverse' : 'row', animation: 'msgIn .26s cubic-bezier(.16,1,.3,1) both' }}>
              <div style={{ width: 26, height: 26, borderRadius: '50%', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 600, marginTop: 3, background: msg.role === 'assistant' ? 'linear-gradient(135deg,#d08a59,#8c6fe0)' : 'var(--ink-700,#14274d)', color: msg.role === 'assistant' ? '#ffffff' : 'var(--tx2)' }}>
                {msg.role === 'assistant'
              ...
              <div style={{ flex: 1, maxWidth: '88%', display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start' }}>
                {msg.role === 'user' ? (
                  <div style={{ padding: '10px 14px', borderRadius: 13, borderBottomRightRadius: 3, background: 'var(--ov)', border: '1px solid var(--b)', fontSize: 13, lineHeight: 1.6, maxWidth: 440 }}>
                    {msg.content}
                  </div>
                ) : (
          ...
          {isLoading && (
            <div style={{ display: 'flex', gap: 9, padding: '2px 0' }}>
              <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'linear-gradient(135deg,#1ed4ca,#9268f8)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="11" height="11" viewBox="0 0 18 18" fill="none" stroke="#04080f" strokeWidth="2.3" strokeLinecap="round"><polyline points="2,14 6,8 10,11 14,4"/></svg>
              </div>
      ...
      {/* Input zone */}
      <div style={{ flexShrink: 0, padding: '12px 16px 16px', borderTop: '1px solid var(--b)', background: 'var(--sf)' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', fontSize: 10, color: 'var(--tx3)', marginTop: 6 }}>
            AskBiz answers BI questions only · Powered by Claude AI · Every answer is grounded in your data
          </div>
        </div>
      </div>
    </div>
