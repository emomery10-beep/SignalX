  ...
  return (
    <div className="page-shell">
      ...
      <div style={{ padding:'clamp(14px,4vw,22px) clamp(14px,3vw,24px) 18px', borderBottom:'1px solid var(--b)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        <div>
          <div style={{ fontFamily:'var(--font-sora)', fontSize:22, fontWeight:600 }}>{tc('files.heading')}</div>
          <div style={{ fontSize:17, color:'var(--tx2)', marginTop:3 }}>{tc('files.subheading')}</div>
        </div>
        <label style={{ display:'flex', alignItems:'center', gap:7, padding:'8px 18px', borderRadius:9999, border:'none', background: uploading ? 'var(--b2)' : '#1ed4ca', color:'#04080f', fontSize:17, fontWeight:600, cursor: uploading ? 'not-allowed' : 'pointer' }}>
      ...
      <div className="page-shell-body">
        ...
        {/* Status message */}
        {uploadMsg && (
          <div style={{ padding:'12px 16px', borderRadius:12, marginBottom:16, background: uploadMsg.startsWith('✓') ? 'rgba(34,197,94,.1)' : uploadMsg.startsWith('✗') ? 'rgba(232,64,64,.1)' : 'rgba(30,212,202,.1)', border: `1px solid ${uploadMsg.startsWith('✓') ? 'rgba(34,197,94,.25)' : uploadMsg.startsWith('✗') ? 'rgba(232,64,64,.25)' : 'rgba(30,212,202,.25)'}`, fontSize:17, color: uploadMsg.startsWith('✓') ? '#22c55e' : uploadMsg.startsWith('✗') ? '#f48080' : '#47e2da' }}>
            {uploadMsg}
          </div>
        )}
        ...
        <label style={{ display:'block', border:'2px dashed var(--b2)', borderRadius:18, padding:48, textAlign:'center', cursor:'pointer', marginBottom:24, transition:'all 200ms' }}
          ...
          <div style={{ fontSize:19, fontWeight:500, marginBottom:5 }}>{uploading ? tc('files.dropzone_uploading') : tc('files.dropzone_idle')}</div>
          <div style={{ fontSize:16, color:'var(--tx3)' }}>{tc('files.dropzone_hint')}</div>
          <input type="file" accept=".csv,.xlsx,.xls" style={{ display:'none' }} disabled={uploading} onChange={e => e.target.files?.[0] && handleUpload(e.target.files[0])}/>
        </label>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
          {uploads.length === 0 && !uploading && (
            <div style={{ textAlign:'center', padding:40, color:'var(--tx3)', fontSize:17 }}>
          ...
          {uploads.map(f => (
            <div key={f.id} style={{ display:'flex', alignItems:'center', gap:12, padding:16, borderRadius:14, border:'1px solid var(--b)', background:'var(--sf)', transition:'border-color 180ms' }}
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:18, fontWeight:500, marginBottom:3, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.filename}</div>
              ...
              <div style={{ display:'flex', gap:8, flexShrink:0, alignItems:'center' }}>
                <span style={{ padding:'3px 10px', borderRadius:9999, background:'rgba(30,212,202,.1)', border:'1px solid rgba(30,212,202,.22)', fontSize:15, color:'#47e2da', fontWeight:500 }}>
                  {f.status === 'parsed' ? tc('files.status_ready') : f.status}
                </span>
                <button onClick={() => useInChat(f.filename)} style={{ padding:'6px 14px', borderRadius:9999, border:'1px solid var(--b2)', background:'transparent', color:'var(--tx)', fontSize:16, cursor:'pointer', fontFamily:'inherit', fontWeight:500 }}>
                  {tc('files.ask_about_this')}
                </button>
                <button onClick={() => deleteFile(f.id)} style={{ padding:'6px 12px', borderRadius:9999, border:'1px solid rgba(232,64,64,.28)', background:'rgba(232,64,64,.08)', color:'#f48080', fontSize:16, cursor:'pointer', fontFamily:'inherit' }}>
                  {tc('files.delete')}
                </button>
              </div>
            </div>
