...
function EmptyState({ colour, message, sub, href }: { colour: string; message: string; sub: string; href: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flex: 1, gap: 8, padding: '8px 0', textAlign: 'center' }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 700, color: TX }}>{message}</div>
      <div style={{ fontSize: 15, color: TX3, lineHeight: 1.4 }}>{sub}</div>
      <a href={href} onClick={e => e.stopPropagation()}
        style={{ fontSize: 15, fontWeight: 700, color: colour, background: colour + '12', border: `1px solid ${colour}30`, padding: '5px 12px', borderRadius: 9999, textDecoration: 'none', marginTop: 2 }}>
        Connect now →
      </a>
    </div>
  )
...
function LivePreview({ id, data, loading }: { id: string; data: PreviewData | null; loading: boolean }) {
  ...
  if (id === 'fx') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ fontSize: 15, fontWeight: 700, color: '#6366F1', textTransform: 'uppercase', letterSpacing: '.06em' }}>Live Import Exposure</div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {data.fx.currencies.map(c => (
            <span key={c.code} style={{ fontSize: 15, padding: '4px 8px', borderRadius: 7, border: '1px solid rgba(99,102,241,.2)', background: 'rgba(99,102,241,.05)', color: '#6366F1', fontWeight: 600 }}>
              {c.flag} {c.code}
            </span>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {data.suppliers.top_three.map(s => (
          <div key={s.name} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 15, color: TX2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '75%' }}>{s.name}</span>
            <span style={{ fontSize: 17, fontWeight: 800, color: s.colour, fontFamily: 'var(--font-sora)', flexShrink: 0 }}>{s.grade}</span>
          </div>
        ))}
        {data.suppliers.at_risk_count > 0 && (
          <div style={{ fontSize: 14, color: '#ea580c', marginTop: 2, fontWeight: 700 }}>
            ⚠ {data.suppliers.at_risk_count} supplier{data.suppliers.at_risk_count > 1 ? 's' : ''} at risk
          </div>
        )}
        <div style={{ fontSize: 14, color: TX3 }}>{data.suppliers.total} total tracked</div>
      </div>
    )
  }
  ...
  if (id === 'landed') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
        <div style={{ fontSize: 14, color: TX3, fontWeight: 600, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {l.sku}
        </div>
        {rows.map(([label, val]) => (
        ...
        <div style={{ borderTop: `1px solid ${B}`, paddingTop: 5, display: 'flex', justifyContent: 'space-between', fontSize: 16 }}>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {data.social.platforms.map(p => (
          <div key={p.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 15 }}>
        ...
        {data.social.demand_signals > 0 && (
          <div style={{ fontSize: 14, color: '#ef4444', fontWeight: 700, marginTop: 2 }}>
  ...
  if (id === 'market') {
    if (data.market) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          <div style={{ fontSize: 14, color: TX3, fontWeight: 600 }}>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <div style={{ padding: '8px 11px', borderRadius: 8, border: `1px solid ${B}`, background: EV, fontSize: 15, color: TX3 }}>
          Search market prices…
        </div>
        <div style={{ fontSize: 14, color: TX3 }}>Expand to search live market prices for any product</div>
      </div>
    )
  }
      ...
      <div className="page-shell">
        <div className="page-shell-body">
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 400, gap: 20, textAlign: 'center', padding: '40px 20px' }}>
            <div style={{ width: 64, height: 64, borderRadius: 20, background: 'rgba(208,138,89,.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 34 }}>📊</div>
            <div style={{ fontFamily: 'var(--font-sora)', fontSize: 26, fontWeight: 700, color: TX }}>Business Intelligence</div>
            <div style={{ fontSize: 18, color: TX2, maxWidth: 360, lineHeight: 1.6 }}>
            ...
            <a href={preview.upgrade_url || '/billing'}
              style={{ padding: '13px 28px', borderRadius: 12, background: ACC, color: '#fff', fontSize: 18, fontWeight: 700, textDecoration: 'none', fontFamily: 'var(--font-sora)' }}>
              Upgrade to Growth →
            </a>
            <div style={{ fontSize: 16, color: TX3 }}>Available on Growth, Business, and Enterprise plans</div>
          </div>
        </div>
      </div>
    )
      ...
      <div className="page-shell-body">
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {[
            ...
            <div key={chip.label} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderRadius: 9999, background: SF, border: `1px solid ${B}`, fontSize: 17 }}>
              <span style={{ color: TX3, fontSize: 16 }}>{chip.label}</span>
          ...
          {(preview?.signals || []).map((sig, i) => (
            <div key={i}
              ...
              style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderRadius: 9999, background: SEV_COL[sig.severity as Severity] + '0f', border: `1px solid ${SEV_COL[sig.severity as Severity]}30`, fontSize: 16, cursor: 'pointer', transition: 'all 160ms ease' }}
              ...
              <span style={{ color: SEV_COL[sig.severity as Severity], fontWeight: 700, fontSize: 15 }}>→</span>
            </div>
          ))}
        </div>
          ...
          {tools.map(tool => (
            <div key={tool.id}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                <div style={{ width: 40, height: 40, borderRadius: 12, background: tool.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 24, flexShrink: 0 }}>
                  {tool.icon}
                </div>
                <div>
                  <div style={{ fontFamily: 'var(--font-sora)', fontSize: 17, fontWeight: 700, color: TX, lineHeight: 1.2 }}>{tool.label}</div>
                  <div style={{ display: 'flex', gap: 4, marginTop: 4, flexWrap: 'wrap' }}>
                    {tool.tags.map(tag => (
                      <span key={tag} style={{ fontSize: 13, fontWeight: 700, color: tool.colour, background: tool.bg, padding: '2px 7px', borderRadius: 9999, textTransform: 'uppercase', letterSpacing: '.06em' }}>{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
              ...
              {/* Expand hint */}
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 16, fontWeight: 600, color: tool.colour }}>
        ...
        <div style={{ padding: '16px 18px', borderRadius: 14, background: EV, border: `1px solid ${B}` }}>
          <div style={{ fontSize: 16, fontWeight: 700, color: TX3, textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {[
              ...
              <button key={i} onClick={() => askAskBiz(prompt)}
                style={{ fontSize: 16, color: ACC, background: 'rgba(208,138,89,.08)', border: '1px solid rgba(208,138,89,.2)', borderRadius: 9999, padding: '6px 13px', cursor: 'pointer', fontFamily: 'inherit' }}>
                {prompt}
              </button>
            ))}
          </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 24px', borderBottom: `1px solid ${B}`, background: activeTool.bg, flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <div style={{ width: 46, height: 46, borderRadius: 14, background: 'rgba(255,255,255,.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 28 }}>
                  {activeTool.icon}
                </div>
                <div>
                  <div style={{ fontFamily: 'var(--font-sora)', fontSize: 21, fontWeight: 700, color: TX }}>{activeTool.label}</div>
                  <div style={{ display: 'flex', gap: 5, marginTop: 4 }}>
                    {activeTool.tags.map(tag => (
                      <span key={tag} style={{ fontSize: 14, fontWeight: 700, color: activeTool.colour, background: activeTool.bg, padding: '2px 8px', borderRadius: 9999, textTransform: 'uppercase', letterSpacing: '.06em' }}>{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
                ...
                <div>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                    <input type="text" value={mktQuery} onChange={e => setMktQuery(e.target.value)}
                      ...
                      style={{ flex: 1, padding: '11px 14px', borderRadius: 10, border: `1.5px solid ${B}`, fontSize: 18, fontFamily: 'inherit', color: TX, outline: 'none', background: EV }} />
                    <button onClick={searchMarket} disabled={mktLoading}
                      style={{ padding: '11px 20px', borderRadius: 10, border: 'none', background: ACC, color: '#fff', fontSize: 18, fontWeight: 600, cursor: mktLoading ? 'not-allowed' : 'pointer', fontFamily: 'inherit', opacity: mktLoading ? 0.7 : 1, whiteSpace: 'nowrap' }}>
                      {mktLoading ? tc('page_tools.marketSearching') : tc('page_tools.marketSearch')}
                    </button>
                  </div>
                  {!mktResult && !mktLoading && <div style={{ fontSize: 17, color: TX3 }}>{tc('page_tools.marketEmptyState')}</div>}
                  {mktResult?.locked && (
                    <div style={{ padding: '20px 18px', borderRadius: 14, border: `1px solid ${ACC}40`, background: `${ACC}08`, textAlign: 'center' }}>
                      <div style={{ fontSize: 28, marginBottom: 8 }}>🌍</div>
                      <div style={{ fontSize: 19, fontWeight: 700, color: TX, marginBottom: 6 }}>Market Intelligence</div>
                      <div style={{ fontSize: 17, color: TX2, marginBottom: 14, lineHeight: 1.5 }}>Available on Growth and above. See real market prices, channel benchmarks, and live web signals.</div>
                      <a href="/billing" style={{ display: 'inline-block', padding: '10px 24px', borderRadius: 9999, background: ACC, color: '#fff', fontSize: 17, fontWeight: 600, textDecoration: 'none' }}>Upgrade to Growth →</a>
                    </div>
                  )}
                  {mktResult?.error && <div style={{ padding: '14px 16px', borderRadius: 10, background: 'rgba(239,68,68,.07)', color: '#ef4444', fontSize: 18 }}>{mktResult.error}</div>}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {(mktResult.products || mktResult.results || []).map((item: any, i: number) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderRadius: 10, border: `1px solid ${B}`, background: EV, gap: 12, flexWrap: 'wrap' }}>
                          <div style={{ fontWeight: 600, fontSize: 18, color: TX, flex: 1 }}>{item.name || item.title || item.product || '—'}</div>
                          <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
                            {(item.price != null || item.price_gbp != null) && (
                              <span style={{ fontFamily: 'var(--font-sora)', fontWeight: 700, fontSize: 19, color: ACC }}>
                                {typeof (item.price ?? item.price_gbp) === 'number' ? `${sym}${(item.price ?? item.price_gbp).toFixed(2)}` : item.price ?? item.price_gbp}
                              </span>
                            )}
                            {item.channel && <span style={{ fontSize: 14, fontWeight: 700, color: '#0284c7', background: 'rgba(2,132,199,.08)', padding: '3px 9px', borderRadius: 9999 }}>{item.channel}</span>}
                            {item.region  && <span style={{ fontSize: 14, fontWeight: 700, color: '#16a34a', background: 'rgba(34,197,94,.08)',  padding: '3px 9px', borderRadius: 9999 }}>{item.region}</span>}
                          </div>
                        </div>
                      ))}
                    </div>
