...
const PAYMENT_BADGE: Record<string, React.ReactNode> = {
  trial: <span title="Free trial, not yet paid" style={{marginLeft:5,padding:'1px 6px',borderRadius:9999,fontSize:11,fontWeight:700,background:'#2563eb20',color:'#2563eb'}}>TRIAL</span>,
...
function KV({ label, value, sub, color, onClick, active }: { label: string; value: any; sub?: string; color?: string; onClick?: () => void; active?: boolean }) {
  ...
  return (
    ...
    >
      <div style={{fontSize:13,fontWeight:600,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:'.07em',lineHeight:1.3,marginBottom:'auto',paddingRight:clickable?22:0}}>{label}</div>
      <div style={{fontSize:32,fontWeight:700,fontFamily:'var(--font-sora)',color:color||'var(--tx)',lineHeight:1.05,letterSpacing:'-.02em',marginTop:14}}>{value}</div>
      {sub && <div style={{fontSize:14,color:'var(--tx2)',marginTop:6,lineHeight:1.4}}>{sub}</div>}
      {clickable && (
  ...
  return (
    <div className="ov-detail" style={{marginBottom:28,borderRadius:16,border:'1px solid '+accent+'44',background:'var(--sf)',overflow:'hidden',boxShadow:'0 8px 28px '+accent+'12'}}>
      <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:12,padding:'16px 22px',borderBottom:'1px solid var(--b)',background:accent+'0d'}}>
        <div>
          <div style={{fontSize:17,fontWeight:700,fontFamily:'var(--font-sora)',color:accent,letterSpacing:'-.01em'}}>{title}</div>
          {subtitle && <div style={{fontSize:14,color:'var(--tx3)',marginTop:3}}>{subtitle}</div>}
        </div>
        <button onClick={onClose} aria-label="Close details" style={{flexShrink:0,width:30,height:30,borderRadius:9,border:'1px solid var(--b)',background:'var(--sf)',color:'var(--tx3)',cursor:'pointer',fontSize:20,lineHeight:1,fontFamily:'inherit',display:'flex',alignItems:'center',justifyContent:'center'}}>×</button>
      </div>
...
function SectionLabel({ children }: { children: any }) {
  return <div style={{fontSize:13,fontWeight:600,color:'var(--tx3)',textTransform:'uppercase',letterSpacing:'.06em',margin:'22px 0 12px'}}>{children}</div>
}

function Chip({ text, color }: { text: string; color: string }) {
  return <span style={{padding:'3px 9px',borderRadius:9999,fontSize:13,fontWeight:600,background:color+'1f',color,textTransform:'capitalize'}}>{text}</span>
}
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(128px,1fr))',gap:12}}>
      {items.map(({label,value}) => (
        <div key={label} style={{padding:'12px 14px',borderRadius:12,background:'var(--bg)',border:'1px solid var(--b)'}}>
          <div style={{fontSize:12,color:'var(--tx3)',marginBottom:6,textTransform:'uppercase',letterSpacing:'.06em'}}>{label}</div>
          <div style={{fontSize:20,fontWeight:700,fontFamily:'var(--font-sora)',letterSpacing:'-.01em'}}>{value}</div>
        </div>
      ))}
    </div>
  )
...
function Bar({ label, pct, color, right }: { label: string; pct: number; color: string; right: string }) {
  return (
    <div style={{marginBottom:14}}>
      <div style={{display:'flex',justifyContent:'space-between',marginBottom:6,fontSize:14.5,gap:8}}>
  ...
  return (
    <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'11px 0',borderBottom:'1px solid var(--b)',gap:12}}>
      <div style={{minWidth:0}}>
        ...
        {sub && <div style={{fontSize:14,color:'var(--tx3)',marginTop:2,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis'}}>{sub}</div>}
      </div>
...
function EmptyNote({ text }: { text: string }) {
  return <div style={{textAlign:'center',padding:'24px 12px',color:'var(--tx3)',fontSize:15}}>{text}</div>
}
      ...
      <DetailShell title={title} subtitle={list.length + ' new ' + (list.length === 1 ? 'signup' : 'signups') + ' in the last ' + days + ' days'} accent={accent} onClose={closeDetail}>
        ...
        {list.length > 0 ? list.slice(0, 25).map(u => (
          <PersonRow key={u.id} name={u.full_name || u.email} sub={u.email + (u.registration_country ? ' · ' + u.registration_country : '')}
            right={<span style={{display:'flex',alignItems:'center',gap:8}}><Chip text={u.plan_id || 'free'} color={PLAN_COLORS[u.plan_id] || '#64748b'} /><span style={{fontSize:14,color:'var(--tx3)',whiteSpace:'nowrap'}}>{fmtDate(u.created_at)}</span></span>} />
            ...
            {mrrByPlanArr.length > 0 ? <>
              ...
              {subs.slice(0, 20).map(s => (
                <PersonRow key={s.id} name={s.customerName || s.customerEmail || '—'} sub={(s.customerEmail || '') + ' · renews ' + fmtDate(s.currentPeriodEnd)}
                  right={<span style={{display:'flex',alignItems:'center',gap:8}}><Chip text={s.plan} color="#6366F1" /><span style={{fontWeight:700,fontSize:15}}>£{s.amount}</span></span>} />
              ))}
        ...
        return (
          <DetailShell title="Annual Run Rate" subtitle="Current MRR projected over 12 months" accent="#7c3aed" onClose={closeDetail}>
            <div style={{display:'flex',alignItems:'baseline',gap:10,flexWrap:'wrap',padding:'4px 0 6px'}}>
              <span style={{fontSize:28,fontWeight:700,fontFamily:'var(--font-sora)'}}>£{mrr}</span>
              <span style={{color:'var(--tx3)',fontSize:15}}>MRR × 12 months =</span>
              <span style={{fontSize:28,fontWeight:700,fontFamily:'var(--font-sora)',color:'#7c3aed'}}>£{arr}</span>
            </div>
            {mrrByPlanArr.length > 0 && <>
        ...
        return (
          <DetailShell title="Free Users" subtitle={freeUsersList.length + ' on the free plan'} accent="#64748b" onClose={closeDetail}>
            ...
            {candidates.length > 0 ? candidates.slice(0, 20).map(c => (
              <PersonRow key={c.id} name={c.full_name || '—'} sub={c.email} right={<span style={{fontSize:14,fontWeight:700,color:'#b45309'}}>{c.questions_used}/10</span>} />
        ...
        return (
          <DetailShell title="Revenue This Month" subtitle={monthPayments.length + ' payment' + (monthPayments.length !== 1 ? 's' : '') + ' since ' + fmtDate(monthStart)} accent="#2563eb" onClose={closeDetail}>
            ...
            {monthPayments.length > 0 ? monthPayments.slice(0, 25).map(p => (
              <PersonRow key={p.id} name={<span>£{p.amount} <span style={{fontSize:13,fontWeight:500,color:p.status === 'succeeded' ? '#047857' : '#dc2626'}}>{p.status}</span></span>} sub={p.email + (p.description ? ' · ' + p.description : '')} right={<span style={{fontSize:14,color:'var(--tx3)',whiteSpace:'nowrap'}}>{fmtDate(p.created)}</span>} />
        ...
        return (
          <DetailShell title="Total Questions Asked" subtitle={totalQuestions + ' questions across ' + users.length + ' users'} accent="#047857" onClose={closeDetail}>
            ...
            {topAskers.length > 0 ? topAskers.slice(0, 25).map(u => (
              <PersonRow key={u.id} name={u.full_name || u.email} sub={u.email} right={<span style={{display:'flex',alignItems:'center',gap:8}}><Chip text={u.plan_id || 'free'} color={PLAN_COLORS[u.plan_id] || '#64748b'} /><span style={{fontWeight:700,fontSize:15}}>{u.questions_used}</span></span>} />
        ...
        return (
          <DetailShell title="Power Users" subtitle={powerUsersList.length + ' user' + (powerUsersList.length !== 1 ? 's' : '') + ' with 5+ questions'} accent="#7c3aed" onClose={closeDetail}>
            {powerUsersList.length > 0 ? powerUsersList.slice(0, 30).map(u => (
              <PersonRow key={u.id} name={u.full_name || u.email} sub={u.email + (u.registration_country ? ' · ' + u.registration_country : '')} right={<span style={{display:'flex',alignItems:'center',gap:8}}><Chip text={u.plan_id || 'free'} color={PLAN_COLORS[u.plan_id] || '#64748b'} /><span style={{fontWeight:700,fontSize:15}}>{u.questions_used}</span></span>} />
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg)',fontFamily:'var(--font-dm,DM Sans,sans-serif)'}}>
      ...
      <div style={{padding:'20px 24px',borderBottom:'1px solid var(--b)',background:'var(--sf)',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div>
          <h1 style={{fontFamily:'var(--font-sora)',fontSize:22,fontWeight:700,margin:0}}>🔐 {tc('admin.title')}</h1>
          <p style={{fontSize:14,color:'var(--tx3)',margin:'2px 0 0'}}>{tc('admin.subtitle')}</p>
        </div>
      ...
      <div style={{padding:'24px',maxWidth:1160,margin:'0 auto'}}>
        {loading ? <div style={{textAlign:'center',padding:60,color:'var(--tx3)'}}>{tc('admin.loading')}</div> : <>
          ...
          {tab==='Overview' && <>
            <div style={{fontSize:14.5,color:'var(--tx3)',marginBottom:14,display:'flex',alignItems:'center',gap:7}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
              <div style={{padding:22,borderRadius:16,border:'1px solid var(--b)',background:'var(--sf)'}}>
                <div style={{fontSize:15,fontWeight:700,letterSpacing:'-.01em',marginBottom:16}}>{tc('admin.plan_distribution')}</div>
              ...
              <div style={{padding:22,borderRadius:16,border:'1px solid var(--b)',background:'var(--sf)'}}>
                <div style={{fontSize:15,fontWeight:700,letterSpacing:'-.01em',marginBottom:16}}>{tc('admin.quick_stats')}</div>
                {[
                  ...
                  <div key={label} style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'11px 0',borderBottom:'1px solid var(--b)',fontSize:15}}>
          ...
          {tab==='Revenue' && <>
            ...
            {stripeData?.subscriptions?.length > 0 && <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:600,marginBottom:12}}>{tc('admin.active_subscriptions')}</div>
                  ...
                  <tbody>
                    {stripeData.subscriptions.map((s:any)=>(
                      <tr key={s.id} style={{borderTop:'1px solid var(--b)'}}>
                        ...
                        <td style={{padding:'8px 12px'}}><span style={{padding:'2px 8px',borderRadius:9999,fontSize:13,fontWeight:600,background:'rgba(99,102,241,.1)',color:'#6366F1'}}>{s.plan}</span></td>
            ...
            {stripeData?.recentPayments?.length > 0 && <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:600,marginBottom:12}}>{tc('admin.recent_payments')}</div>
              {stripeData.recentPayments.map((p:any)=>(
                <div key={p.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid var(--b)',flexWrap:'wrap',gap:8}}>
                  <div>
                    <div style={{fontWeight:600,fontSize:15}}>£{p.amount} <span style={{fontSize:13,color:p.status==='succeeded'?'#047857':'#dc2626',fontWeight:500}}>{p.status}</span></div>
                    <div style={{fontSize:14,color:'var(--tx3)'}}>{p.email}{p.description?' · '+p.description:''}</div>
                  </div>
                  <div style={{fontSize:13,color:'var(--tx3)'}}>{new Date(p.created).toLocaleDateString('en-GB')}</div>
                </div>
              ))}
            </div>}

            <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)'}}>
              <div style={{fontSize:15,fontWeight:600,marginBottom:8}}>{tc('admin.upgrade_candidates_title', { count: candidates.length })}</div>
              <p style={{fontSize:14,color:'var(--tx3)',marginBottom:16}}>{tc('admin.upgrade_candidates_desc')}</p>
              {candidates.length===0 ? <div style={{textAlign:'center',padding:20,color:'var(--tx3)'}}>{tc('admin.no_candidates')}</div> :
                candidates.map(c => (
                  <div key={c.id} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 0',borderBottom:'1px solid var(--b)',flexWrap:'wrap',gap:8}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:15}}>{c.full_name||tc('admin.default_user')}</div>
                      <div style={{fontSize:14,color:'var(--tx3)'}}>{c.email} · {c.questions_used}/10</div>
                    </div>
                    <button onClick={() => changePlan(c.id,'growth')} style={{padding:'6px 14px',borderRadius:9999,border:'none',background:'#b45309',color:'#fff',fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>{tc('admin.give_growth')}</button>
                  </div>
                ))
              }
            </div>
          ...
          {tab==='Activity' && <>
            ...
            <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
              <div style={{fontSize:15,fontWeight:600,marginBottom:16}}>{tc('admin.daily_signups')}</div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
              <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)'}}>
                <div style={{fontSize:15,fontWeight:600,marginBottom:12}}>{tc('admin.by_country')}</div>
                {Object.entries(users.reduce((acc:any,u)=>{const c=u.registration_country||tc('admin.unknown');acc[c]=(acc[c]||0)+1;return acc},{})).sort((a:any,b:any)=>b[1]-a[1]).slice(0,8).map(([c,n]:any)=>(
                  <div key={c} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid var(--b)',fontSize:14}}>
                    <span>{c}</span><span style={{fontWeight:600}}>{n}</span>
                  </div>
                ))}
              </div>
              <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)'}}>
                <div style={{fontSize:15,fontWeight:600,marginBottom:12}}>{tc('admin.by_business_type')}</div>
                {Object.entries(users.reduce((acc:any,u)=>{const t=u.business_type||tc('admin.unknown');acc[t]=(acc[t]||0)+1;return acc},{})).sort((a:any,b:any)=>b[1]-a[1]).map(([t,n]:any)=>(
                  <div key={t} style={{display:'flex',justifyContent:'space-between',padding:'6px 0',borderBottom:'1px solid var(--b)',fontSize:14}}>
            ...
            return <>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(130px,1fr))',gap:10,marginBottom:16}}>
                {[
                  ...
                  <div key={label} style={{padding:'10px 12px',borderRadius:9,background:'var(--sf)',border:'1px solid var(--b)'}}>
                    <div style={{fontSize:12,color:'var(--tx3)',marginBottom:4,textTransform:'uppercase',letterSpacing:'.06em'}}>{label}</div>
                    <div style={{fontSize:17,fontWeight:700,fontFamily:'var(--font-sora)'}}>{value}</div>
                  </div>
                ))}
              </div>
              ...
              {Object.keys(byDay).length > 0 && (
                <div style={{padding:'16px 20px',borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:600,color:'var(--tx3)',marginBottom:10,textTransform:'uppercase',letterSpacing:'.06em'}}>Spend — last 30 days</div>
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',fontSize:12,color:'var(--tx3)',marginTop:4}}>
              ...
              {Object.keys(byModel).length > 0 && (
                <div style={{padding:'16px 20px',borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:600,color:'var(--tx3)',marginBottom:10,textTransform:'uppercase',letterSpacing:'.06em'}}>By model</div>
                    ...
                    return <div key={model} style={{marginBottom:10}}>
                      <div style={{display:'flex',justifyContent:'space-between',fontSize:14,marginBottom:4}}>
                        <span style={{display:'flex',alignItems:'center',gap:6}}>
                          <span style={{padding:'1px 5px',borderRadius:99,background:groq?'#f9731618':'#7c3aed18',color:groq?'#f97316':'#7c3aed',fontSize:12,fontWeight:700}}>{groq?'Groq':'Anthropic'}</span>
                          <span style={{padding:'1px 7px',borderRadius:99,background:col+'22',color:col,fontSize:13,fontWeight:600}}>{modelLabel(model)}</span>
              ...
              {catTotals.length > 0 && (
                <div style={{padding:'16px 20px',borderRadius:14,border:'1px solid rgba(99,102,241,.3)',background:'rgba(99,102,241,.04)',marginBottom:16}}>
                  <div style={{fontSize:13,fontWeight:600,color:'#6366F1',marginBottom:12,textTransform:'uppercase',letterSpacing:'.06em'}}>Breakdown by route</div>
                  {catTotals.map(({cat,subtotal})=>{
                    ...
                    return <div key={cat} style={{marginBottom:16}}>
                      ...
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                        ...
                        <span style={{fontSize:14,fontWeight:600,color:'var(--tx)'}}>${subtotal.toFixed(4)} <span style={{color:'var(--tx3)',fontWeight:400}}>({Math.round(pct)}%)</span></span>
                      </div>
                      {/* Category bar */}
                        ...
                        return <div key={route} style={{display:'grid',gridTemplateColumns:'1fr auto',gap:8,padding:'7px 10px',borderRadius:8,background:'var(--sf)',border:'1px solid var(--b)',marginBottom:5,alignItems:'center'}}>
                          <div>
                            <div style={{display:'flex',alignItems:'center',gap:4,marginBottom:3,flexWrap:'wrap'}}>
                              <span style={{fontSize:13,fontFamily:'monospace',color:'var(--tx2)'}}>{route}</span>
                              {d.model && <>
                                <span style={{padding:'0px 4px',borderRadius:99,background:isGroq(d.model)?'#f9731618':'#7c3aed18',color:isGroq(d.model)?'#f97316':'#7c3aed',fontSize:11,fontWeight:700,whiteSpace:'nowrap'}}>{isGroq(d.model)?'Groq':'Anthropic'}</span>
                                <span style={{padding:'0px 5px',borderRadius:99,background:col+'22',color:col,fontSize:12,fontWeight:600,whiteSpace:'nowrap'}}>{modelLabel(d.model)}</span>
                              </>}
                            </div>
                          ...
                          <div style={{textAlign:'right',whiteSpace:'nowrap'}}>
                            <div style={{fontSize:14,fontWeight:600,color:'var(--tx)'}}>${d.costUsd.toFixed(4)}</div>
                            <div style={{fontSize:12,color:'var(--tx3)'}}>{d.calls} calls</div>
                          </div>
                        </div>
                      })}
                    </div>
              ...
              {(!apiUsage || Object.keys(byRoute).length===0) && (
                <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16,fontSize:14,color:'var(--tx3)'}}>
              ...
              {/* ── Other monthly costs ── */}
              <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)',marginBottom:16}}>
                <div style={{fontSize:15,fontWeight:600,marginBottom:4}}>{tc('admin.other_monthly_costs')}</div>
                <p style={{fontSize:14,color:'var(--tx3)',marginBottom:16}}>{tc('admin.other_monthly_costs_desc')}</p>
                {[
                  ...
                  <div key={service} style={{display:'flex',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid var(--b)',flexWrap:'wrap',gap:8}}>
                    <div>
                      <div style={{fontWeight:600,fontSize:15,color}}>{service}</div>
                      <div style={{fontSize:13,color:'var(--tx3)',marginTop:2}}>{note}</div>
                    </div>
                    <div style={{fontSize:15,fontWeight:600,color:'var(--tx2)'}}>{estimate}</div>
                  </div>
                ))}
              </div>
              ...
              <div style={{padding:20,borderRadius:14,border:'1px solid rgba(99,102,241,.2)',background:'rgba(99,102,241,.04)'}}>
                <div style={{fontSize:15,fontWeight:600,color:'#6366F1',marginBottom:12}}>{tc('admin.unit_economics')}</div>
            ...
            <div style={{padding:20,borderRadius:14,border:'1px solid var(--b)',background:'var(--sf)'}}>
              ...
              {[
                ...
                <div key={action} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 0',borderBottom:'1px solid var(--b)',flexWrap:'wrap',gap:8}}>
                  <div>
                    <div style={{fontWeight:600,fontSize:15,color}}>{action}</div>
                    <div style={{fontSize:14,color:'var(--tx3)',marginTop:2}}>{detail}</div>
                  </div>
                  <button onClick={onClick} style={{padding:'6px 14px',borderRadius:9999,border:'1px solid '+color,background:'transparent',color,fontSize:14,fontWeight:600,cursor:'pointer',fontFamily:'inherit'}}>{cta}</button>
                </div>
              ))}
            </div>
          </>}
