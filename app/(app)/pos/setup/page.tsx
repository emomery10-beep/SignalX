...
export default function PosSetupPage() {
  ...
  const bigBtn: React.CSSProperties = {
    ...
    background: ACC, color: '#fff', fontSize: 19, fontWeight: 700,
  ...
  const ghostBtn: React.CSSProperties = {
    ...
    background: 'transparent', color: TX2, fontSize: 18, fontWeight: 600,
      ...
      <div className="pos-setup-shell" style={{ width: '100%', maxWidth: 480, margin: '0 auto', padding: '20px 16px 40px', flex: 1 }}>
            ...
            {undoItem && (
              <div role="status" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '12px 15px', borderRadius: 12, background: TX, color: '#fff', marginBottom: 16 }}>
                ...
                <button onClick={() => undoRemove(undoItem)}
                  style={{ flexShrink: 0, padding: '8px 16px', borderRadius: 9999, border: 'none', background: ACC, color: '#fff', fontSize: 16, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                  {tc('pos_setup.undo')}
                </button>
              </div>
            )}
              ...
              <>
                ...
                <button onClick={openImport}
                  ...
                  <span style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ display: 'block', fontSize: 18, fontWeight: 700, color: TX }}>{tc('pos_setup.fork_have_title')}</span>
                    <span style={{ display: 'block', fontSize: 15, color: TX2, marginTop: 2, lineHeight: 1.45 }}>{tc('pos_setup.fork_have_desc')}</span>
                  </span>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                  {items.map(it => (
                    <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 12, background: SF, border: `1px solid ${B}` }}>
                      ...
                      <span style={{ fontSize: 17, fontWeight: 600, color: TX, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{it.name}</span>
                      <span style={{ fontSize: 17, fontWeight: 700, color: ACC, flexShrink: 0 }}>{currencySymbol}{it.sale_price}</span>
          ...
          <>
            ...
            {importPhase === 'camera' && (
              <>
                ...
                <p style={{ fontSize: 17, color: TX2, lineHeight: 1.6, marginBottom: 16 }}>
                ...
                <div style={{ position: 'relative', width: '100%', aspectRatio: '4 / 3', borderRadius: 16, overflow: 'hidden', background: '#000', marginBottom: 16 }}>
                  <video ref={videoRef} playsInline muted autoPlay style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  {importBusy && (
                    <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 17, fontWeight: 600 }}>
            ...
            {importPhase === 'review' && (
              <>
                ...
                <p style={{ fontSize: 16, color: TX2, lineHeight: 1.6, marginBottom: 20 }}>
                  ...
                  {importItems.map((it, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 12, background: SF, border: `1px solid ${B}` }}>
                      <input
                        ...
                        placeholder={tc('pos_setup.name_placeholder')}
                        style={{ flex: 1, minWidth: 0, padding: '10px 12px', fontSize: 17, background: EV, border: `1.5px solid ${B2}`, borderRadius: 10, color: TX, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
                      />
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexShrink: 0 }}>
                        <span style={{ fontSize: 17, fontWeight: 700, color: TX2 }}>{currencySymbol}</span>
                        <input
                          value={it.sale_price ? String(it.sale_price) : ''} onChange={e => editImportItem(i, { sale_price: parseFloat(e.target.value.replace(/[^\d.]/g, '')) || 0 })}
                          inputMode="decimal" placeholder="0"
                          style={{ width: 72, padding: '10px 8px', fontSize: 17, fontWeight: 700, textAlign: 'right', background: EV, border: `1.5px solid ${B2}`, borderRadius: 10, color: TX, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }}
                        />
                      </div>
        ...
        {screen === 'team' && (
          <>
            ...
            <p style={{ fontSize: 17, color: TX2, lineHeight: 1.6, marginBottom: 20 }}>
            ...
            {!alreadyEnabled ? (
              <div style={{ padding: '14px 16px', borderRadius: 14, background: SF, border: `1px solid ${B}`, marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 16, color: TX2 }}>{seatCount === 1 ? tc('pos_setup.team_seats_one') : tc('pos_setup.team_seats', { seats: seatCount })}</span>
                <span style={{ fontSize: 18, fontWeight: 700, color: ACC }}>{posSeatPrice(currency, seatCount)}<span style={{ fontSize: 14, color: TX3, fontWeight: 500 }}>{tc('pos_setup.team_per_month')}</span></span>
              </div>
            ) : pendingSeats > 0 ? (
              <div style={{ padding: '14px 16px', borderRadius: 14, background: 'rgba(208,138,89,.08)', border: `1px solid ${ACC}`, marginBottom: 16 }}>
                <div style={{ fontSize: 16, fontWeight: 600, color: TX }}>{tc('pos_setup.team_pending_count', { count: pendingSeats })}</div>
                <div style={{ fontSize: 15, color: TX2, marginTop: 2 }}>{tc('pos_setup.team_pending_note', { price: posSeatPrice(currency, pendingSeats) })}</div>
              </div>
            ) : null}
                ...
                {drafts.map(d => (
                  <div key={d.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', borderRadius: 12, background: SF, border: `1px solid ${B}` }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 17, fontWeight: 600, color: TX, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</div>
                      <div style={{ fontSize: 14, color: TX3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tc(`pos_setup.role_${d.role}`)} · {d.phone || d.email}</div>
                    </div>
                    {alreadyEnabled && (
                      <span style={{ flexShrink: 0, fontSize: 13, fontWeight: 700, color: ACC, background: 'rgba(208,138,89,.12)', borderRadius: 9999, padding: '3px 9px' }}>{tc('pos_setup.team_pending_badge')}</span>
                    )}
        ...
        {screen === 'team-add' && (
          <>
            ...
            <label style={{ fontSize: 15, fontWeight: 600, color: TX2, display: 'block', marginBottom: 6 }}>{tc('pos_setup.team_name_label')}</label>
            <input value={memberName} onChange={e => setMemberName(e.target.value)} placeholder={tc('pos_setup.team_name_placeholder')}
              style={{ width: '100%', padding: '13px 15px', fontSize: 18, background: EV, border: `1.5px solid ${B2}`, borderRadius: 12, color: TX, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box', marginBottom: 16 }} />

            <label style={{ fontSize: 15, fontWeight: 600, color: TX2, display: 'block', marginBottom: 6 }}>{tc('pos_setup.team_role_label')}</label>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              {(['cashier', 'inventory'] as const).map(r => (
                <button key={r} onClick={() => setMemberRole(r)}
                  style={{ flex: 1, padding: '12px', borderRadius: 12, border: `1.5px solid ${memberRole === r ? ACC : B2}`, background: memberRole === r ? 'rgba(208,138,89,.08)' : 'transparent', color: memberRole === r ? ACC : TX2, fontFamily: 'inherit', fontSize: 16, fontWeight: memberRole === r ? 700 : 500, cursor: 'pointer' }}>
            ...
            <label style={{ fontSize: 15, fontWeight: 600, color: TX2, display: 'block', marginBottom: 6 }}>{tc('pos_setup.team_login_label')}</label>
            <div style={{ position: 'relative', display: 'flex', background: EV, borderRadius: 10, padding: 3, marginBottom: 10 }}>
              ...
              {(['phone', 'email'] as const).map(m => (
                <button key={m} onClick={() => { setMemberMethod(m); setTeamError('') }}
                  style={{ position: 'relative', zIndex: 1, flex: 1, padding: '8px', borderRadius: 8, border: 'none', background: 'transparent', color: memberMethod === m ? TX : TX2, fontFamily: 'inherit', fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>
            ...
            {memberMethod === 'phone' ? (
              <div style={{ display: 'flex', gap: 6, marginBottom: 16 }} dir="ltr">
                <select value={memberCountry} onChange={e => setMemberCountry(e.target.value)} aria-label={tc('pos_setup.team_method_phone')}
                  style={{ width: 92, flexShrink: 0, padding: '13px 8px', borderRadius: 12, border: `1.5px solid ${B2}`, fontSize: 17, fontFamily: 'inherit', background: EV, color: TX, cursor: 'pointer', appearance: 'none' }}>
                ...
                <input value={memberPhone} onChange={e => setMemberPhone(e.target.value)} type="tel" inputMode="tel" dir="ltr" placeholder="712 345678"
                  style={{ flex: 1, minWidth: 0, padding: '13px 15px', fontSize: 18, background: EV, border: `1.5px solid ${B2}`, borderRadius: 12, color: TX, outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
              </div>
            ) : (
            ...
            <label style={{ fontSize: 15, fontWeight: 600, color: TX2, display: 'block', marginBottom: 6 }}>{tc('pos_setup.team_pin_label')}</label>
