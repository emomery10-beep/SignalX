  ...
  return (
    <div className="page-shell">
      ...
      <div className="page-shell-body">
        <div style={{ display: 'grid', gridTemplateColumns: 'min(280px,100%) 1fr', gap: 20 }}>
                ...
                <div style={formGroup}>
                  ...
                  <select style={selectStyle} value={targetColumn} onChange={e => setTargetColumn(e.target.value)}>
                    ...
                    {numericColumns.length > 0 && (
                      <optgroup label="Numeric (recommended)">
                        {numericColumns.map(c => <option key={c} value={c}>{c}</option>)}
                      </optgroup>
                    )}
                    {selectedUpload.column_names?.filter(c => !numericColumns.includes(c)).length > 0 && (
                      <optgroup label="Text columns (will auto-detect best numeric)">
                        {selectedUpload.column_names?.filter(c => !numericColumns.includes(c)).map(c => <option key={c} value={c}>{c}</option>)}
                      </optgroup>
                    )}
                  </select>
                  {targetColumn && !numericColumns.includes(targetColumn) && (
                    <div style={{ marginTop: 5, fontSize: 11, color: '#f59e0b', lineHeight: 1.5 }}>
                      ⚠ &quot;{targetColumn}&quot; is a text column. AskBiz will automatically forecast your best numeric column instead.
                    </div>
                  )}
                  {numericColumns.length === 0 && selectedUpload && (
                    <div style={{ marginTop: 5, fontSize: 11, color: 'var(--tx3)', lineHeight: 1.5 }}>
