          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            {result?.dataQuality && (
              <div style={card}>
                ...
                <div style={{ marginTop: 8, fontSize: 11, color: 'var(--tx3)' }}>{tc('forecasts.points_outliers', { rows: result.dataQuality.rowCount, outliers: result.dataQuality.outliers, plural: result.dataQuality.outliers !== 1 ? 's' : '' })}</div>
              </div>
            )}
            ...
            {result?.anomalies && result.anomalies.length > 0 && (
              <div style={{ ...card, borderColor: 'rgba(244,128,128,.3)' }}>
                ...
                {result.anomalies.length > 5 && <div style={{ fontSize: 11, color: 'var(--tx3)', marginTop: 6 }}>{tc('forecasts.more_count', { count: result.anomalies.length - 5 })}</div>}
              </div>
            )}
            ...
            {savedForecasts.length > 0 && (
              <div style={card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                  ...
                  {canUsePro && result && <span style={{ fontSize: 10, color: 'var(--tx3)' }}>{tc('forecasts.toggle_to_overlay')}</span>}
                </div>
          ...
          <div>
            {!result ? (
              <div style={{ background: 'var(--sf)', border: '1px solid var(--b)', borderRadius: 14, padding: 48, textAlign: 'center', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 14, minHeight: 440 }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 600, fontFamily: 'var(--font-sora)' }}>{tc('forecasts.empty_title')}</div>
                ...
                {activeTab === 'overview' && (
                  <>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', borderRadius: 10, background: 'var(--sf)', border: '1px solid var(--b)', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 12, color: 'var(--tx2)', fontWeight: 500 }}>{tc('forecasts.scenario_label')}</span>
                  ...
                  <div style={{ ...card, padding: 18 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: 'var(--tx3)', marginTop: 4 }}>{tc('forecasts.method_comparison_desc')}</div>
                      </div>
    ...
    {showShareModal && (
      <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }} onClick={() => setShowShareModal(false)}>
        <div style={{ background: 'var(--sf)', borderRadius: 16, padding: 24, maxWidth: 420, width: '90%', boxShadow: '0 20px 60px rgba(0,0,0,.2)' }} onClick={e => e.stopPropagation()}>
          <div style={{ fontFamily: 'var(--font-sora)', fontSize: 16, fontWeight: 600, marginBottom: 8 }}>{tc('forecasts.share_forecast')}</div>
