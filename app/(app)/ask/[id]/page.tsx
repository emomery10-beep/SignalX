...
function buildBizQuestions(tc: (key: string) => string): Record<string, string[]> {
  return {
    retail:      [tc('page_ask_id.bizRetail0'), tc('page_ask_id.bizRetail1'), tc('page_ask_id.bizRetail2'), tc('page_ask_id.bizRetail3')],
    ecommerce:   [tc('page_ask_id.bizEcommerce0'), tc('page_ask_id.bizEcommerce1'), tc('page_ask_id.bizEcommerce2'), tc('page_ask_id.bizEcommerce3')],
    distributor: [tc('page_ask_id.bizDistributor0'), tc('page_ask_id.bizDistributor1'), tc('page_ask_id.bizDistributor2'), tc('page_ask_id.bizDistributor3')],
    exporter:    [tc('page_ask_id.bizExporter0'), tc('page_ask_id.bizExporter1'), tc('page_ask_id.bizExporter2'), tc('page_ask_id.bizExporter3')],
  }
}
...
export default function ChatConversationPage() {
  ...
  const [convTitle, setConvTitle] = useState(tc('page_ask_id.newConversation'))
  ...
  const sendMessage = useCallback(async (text?: string) => {
    ...
    try {
      ...
      setMessages(m => [...m, {
        id: (Date.now() + 1).toString(), role: 'assistant',
        content: tc('page_ask_id.aiError'),
        timestamp: new Date()
      }])
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100vh', overflow:'hidden' }}>
      <div style={{ height:50, padding:'0 18px', borderBottom:'1px solid var(--b)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0, background:'var(--sf)', gap:10 }}>
        <div style={{ fontSize:15, fontWeight:500, flex:1, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
          {isEmpty ? tc('page_ask_id.newConversation') : convTitle}
        </div>
        {uploadedFile && (
        ...
        <div style={{ display:'flex', gap:7, flexShrink:0 }}>
          <label style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 13px', borderRadius:9999, border:'1px solid var(--b2)', background:'transparent', color:'var(--tx)', fontSize:14, cursor:'pointer', whiteSpace:'nowrap' }}>
            ...
            {uploading ? tc('page_ask_id.uploadingLabel') : tc('page_ask_id.uploadLabel')}
        ...
        <div style={{ maxWidth: 'min(680px, 100%)', margin:'0 auto', padding:'0 18px', display:'flex', flexDirection:'column', gap:2 }}>
          {isEmpty && (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:400, textAlign:'center', padding:28 }}>
              <div style={{ fontFamily:'var(--font-sora)', fontSize:21, fontWeight:600, marginBottom:7 }}>{tc('page_ask_id.emptyHeading')}</div>
              <div style={{ fontSize:15, color:'var(--tx2)', lineHeight:1.65, maxWidth:360, marginBottom:20 }}>
                {geo ? tc('page_ask_id.emptySubGeo').replace('{symbol}', geo.currencySymbol).replace('{country}', geo.country) : tc('page_ask_id.emptySubDefault')}
              </div>
              <div style={{ display:'flex', flexDirection:'column', gap:2, width:'100%', maxWidth:420, textAlign:'left' }}>
                {suggestions.map((q, i) => (
                  <button key={i} onClick={() => sendMessage(q)}
                    ...
                    <span style={{ fontSize:15, color:'var(--tx2)', lineHeight:1.4 }}>{q}</span>
                  </button>
                ))}
              </div>
            </div>
