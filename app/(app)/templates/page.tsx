...
export default function TemplatesPage() {
  ...
  return (
    <div className="page-shell">
      <div style={{ padding:'clamp(14px,4vw,20px) clamp(14px,3vw,24px) 16px', borderBottom:'1px solid var(--b)', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
        <div>
      ...
      <div className="page-shell-body">
        {loading ? (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:16 }}>
            {[1,2,3,4].map(i => <div key={i} style={{ height:200, borderRadius:14, background:'var(--ev)', animation:'shimmer 1.4s infinite', backgroundSize:'200% 100%' }}></div>)}
          </div>
        ) : templates.length === 0 ? (
