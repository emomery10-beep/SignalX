  ...
  return (
    <div style={{
      ...
      <nav style={{
        ...
        <Link href={localePath('/', lang)} style={{ display: 'flex', alignItems: 'center', gap: 9, textDecoration: 'none' }}>
          <div style={{ width: 28, height: 28, borderRadius: 8, background: '#6366F1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <svg width="14" height="14" viewBox="0 0 32 32" fill="none"><g fill="none" stroke="white" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round"><path d="M5 11 V5 H11"/><path d="M21 5 H27 V11"/><path d="M5 21 V27 H11"/><path d="M27 21 V27 H21"/></g><circle cx="16" cy="16" r="2.6" fill="white"/></svg>
          </div>
          <span style={{ fontSize: 14, fontWeight: 700, color: '#0F172A', letterSpacing: '-.02em' }}>
      ...
      <main style={{ maxWidth: 680, margin: '0 auto', padding: '40px 20px 80px' }}>
        ...
        {/* Meta: date + views */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, fontSize: 10, color: '#94A3B8' }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, fontWeight: 700, color: '#94A3B8', letterSpacing: '.08em', marginBottom: 10, textTransform: 'uppercase' }}>
            Question asked
          </div>
        ...
        {insight.insight_header && (
          <div style={{
            ...
            <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: '#1E293B', lineHeight: 1.6 }}>
              {insight.insight_header}
            </p>
          </div>
        )}
          ...
          <div style={{
            ...
            {(insight.kpi_cards as KpiCard[]).map((card, i) => (
              <div key={i} style={{
                ...
                <div style={{ fontSize: 9, fontWeight: 700, color: '#94A3B8', letterSpacing: '.07em', textTransform: 'uppercase', marginBottom: 6 }}>
                  {card.label}
                </div>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 5 }}>
                  <span style={{ fontSize: 18, fontWeight: 800, color: card.status ? statusColor(card.status) : '#0F172A', letterSpacing: '-.02em' }}>
                    {card.value}
                  </span>
                  {card.trend && (
                    <span style={{ fontSize: 11, color: trendColor(card.trend), fontWeight: 600 }}>
        ...
        {insight.recommendations && insight.recommendations.length > 0 && (
          <div style={{
            ...
            <div style={{ fontSize: 9, fontWeight: 700, color: '#94A3B8', letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 14 }}>
              🎯 Recommendations
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {(insight.recommendations as string[]).map((rec, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: 12, color: '#475569', lineHeight: 1.6 }}>{rec}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.1em', opacity: 0.75, marginBottom: 10, textTransform: 'uppercase' }}>
            Powered by AskBiz
          </div>
          <div style={{ fontSize: 20, fontWeight: 800, marginBottom: 8, letterSpacing: '-.02em', lineHeight: 1.3 }}>
            Want insights like this<br/>for your own business?
          </div>
          <div style={{ fontSize: 12, opacity: 0.82, marginBottom: 24, lineHeight: 1.6 }}>
          ...
          <div style={{ fontSize: 10, opacity: 0.65, marginTop: 12 }}>
