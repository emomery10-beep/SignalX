...
export const metadata: Metadata = {
  ...
  openGraph: {
    title: 'Human-Friendly Alternative to Bloomberg Terminal | AskBiz',
    description: 'Bloomberg translates financial data for institutions. AskBiz translates YOUR business data into plain English. No jargon, no £24,000/year terminal. From £19/month.',
    url: 'https://askbiz.co/bloomberg-alternative',
    siteName: 'AskBiz',
    type: 'website',
    images: [{ url: 'https://askbiz.co/api/og?title=Bloomberg+Alternative+for+SMEs&category=AskBiz', width: 1200, height: 630, alt: 'Human-Friendly Bloomberg Alternative — AskBiz' }],
  },
  ...
  return (
    <div style={{ fontFamily: 'var(--font-dm, system-ui)', color: '#1a1916', background: '#f9f8f6' }}>
      ...
      <nav style={{ borderBottom: '1px solid #e8e6e1', background: '#fff', padding: '0 clamp(16px,4vw,24px)', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <Link href={localePath('/signin', lang)} style={{ fontSize: 11, fontWeight: 600, color: '#fff', background: '#d08a59', borderRadius: 9999, padding: '7px 18px', textDecoration: 'none' }}>Try free →</Link>
      </nav>
      ...
      <section style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(48px,8vw,80px) clamp(16px,4vw,24px) clamp(32px,5vw,56px)', textAlign: 'center' }}>
        <div style={{ display: 'inline-block', fontSize: 10, fontWeight: 600, color: '#8c6fe0', background: 'rgba(140,111,224,.1)', border: '1px solid rgba(140,111,224,.25)', borderRadius: 9999, padding: '4px 14px', marginBottom: 20, letterSpacing: '.04em', textTransform: 'uppercase' }}>
        ...
        <p style={{ fontSize: 12, color: '#a39e97', marginBottom: 32 }}>From £19/month. Works with any CSV or Excel file.</p>
        <Link href={localePath('/signin', lang)} style={{ fontSize: 13, fontWeight: 600, color: '#fff', background: '#d08a59', borderRadius: 9999, padding: '13px 32px', textDecoration: 'none', display: 'inline-block' }}>
      ...
      <section style={{ background: '#fff', borderTop: '1px solid #e8e6e1', borderBottom: '1px solid #e8e6e1', padding: 'clamp(40px,6vw,64px) clamp(16px,4vw,24px)' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 13, color: '#6b6760', textAlign: 'center', marginBottom: 36 }}>This is what financial analysis looks like before and after AskBiz.</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, borderRadius: 16, overflow: 'hidden', border: '1px solid #e8e6e1' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', background: '#1a1916' }}>
              <div style={{ padding: '12px 20px', fontSize: 10, fontWeight: 700, color: '#a39e97', textTransform: 'uppercase', letterSpacing: '.08em', borderRight: '1px solid #333' }}>
                🖥️ Terminal / Financial jargon
              </div>
              <div style={{ padding: '12px 20px', fontSize: 10, fontWeight: 700, color: '#d08a59', textTransform: 'uppercase', letterSpacing: '.08em' }}>
            ...
            {comparisons.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderTop: '1px solid #e8e6e1', background: i % 2 === 0 ? '#fff' : '#f9f8f6' }}>
                <div style={{ padding: '16px 20px', color: '#6b6760', lineHeight: 1.6, borderRight: '1px solid #e8e6e1', fontFamily: 'var(--font-mono, monospace)', fontSize: 11 }}>
                  {row.terminal}
                </div>
                <div style={{ padding: '16px 20px', fontSize: 12, color: '#1a1916', lineHeight: 1.65 }}>
                  {row.askbiz}
                </div>
              </div>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 16 }}>
          {[
            ...
            <div key={i} style={{ padding: '18px 20px', borderRadius: 14, border: '1px solid #e8e6e1', background: '#fff' }}>
              ...
              <div style={{ fontFamily: 'var(--font-sora, system-ui)', fontSize: 12, fontWeight: 600, marginBottom: 6 }}>{c.title}</div>
              <p style={{ fontSize: 11, color: '#6b6760', lineHeight: 1.6, margin: 0 }}>{c.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ background: '#fff', borderTop: '1px solid #e8e6e1', borderBottom: '1px solid #e8e6e1', padding: 'clamp(40px,6vw,64px) clamp(16px,4vw,24px)' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.85, color: '#4a4844', marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, lineHeight: 1.85, color: '#4a4844' }}>
      ...
      <section style={{ maxWidth: 760, margin: '0 auto', padding: 'clamp(48px,8vw,72px) clamp(16px,4vw,24px)', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 14, color: '#6b6760', lineHeight: 1.7, marginBottom: 28, maxWidth: 480, margin: '0 auto 28px' }}>Upload your CSV, ask your question, get your answer. No financial jargon. No data science degree. No terminal subscription.</p>
        <Link href={localePath('/signin', lang)} style={{ fontSize: 14, fontWeight: 600, color: '#fff', background: '#d08a59', borderRadius: 9999, padding: '14px 36px', textDecoration: 'none', display: 'inline-block' }}>
          Start free — no card needed →
        </Link>
        <p style={{ fontSize: 10, color: '#a39e97', marginTop: 14 }}>10 free questions every month. Growth from £19/month.</p>
      </section>
      ...
      <section style={{ borderTop: '1px solid #e8e6e1', background: '#fff', padding: 'clamp(28px,4vw,40px) clamp(16px,4vw,24px)' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: '#a39e97', textTransform: 'uppercase', letterSpacing: '.08em', marginBottom: 16 }}>Financial terms in plain English</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
            {[
              ...
              <Link key={slug} href={localePath(`/translate/${slug}`, lang)} style={{ fontSize: 11, color: '#8c6fe0', textDecoration: 'none', padding: '6px 14px', borderRadius: 9999, border: '1px solid rgba(140,111,224,.25)', background: 'rgba(140,111,224,.04)' }}>
                {label} →
              </Link>
            ))}
          </div>
      ...
      <footer style={{ borderTop: '1px solid #e8e6e1', padding: 'clamp(20px,3vw,32px) clamp(16px,4vw,24px)', display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 16 }}>
          {[['/', 'Home'], ['/translate', 'Glossary'], ['/how-to', 'How-to'], ['/privacy', 'Privacy']].map(([href, label]) => (
            <Link key={href} href={localePath(href, lang)} style={{ fontSize: 11, color: '#6b6760', textDecoration: 'none' }}>{label}</Link>
          ))}
        </div>
      </footer>
    </div>
