...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // After sync completes, recalculate health score from unified_data
    const totalSynced = filtered.reduce((s, r) => s + r.recordsSynced, 0)
    if (totalSynced > 0) {
      ...
      if (unifiedRows && unifiedRows.length > 0) {
        ...
        // Call health API internally
        const appUrl = process.env.NEXT_PUBLIC_APP_URL || 'https://askbiz.co'
        fetch(`${appUrl}/api/health`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-internal': 'sync' },
          body: JSON.stringify({ rows, headers, userId: user.id }),
        }).catch(() => {})
      }
    }
