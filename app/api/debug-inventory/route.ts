...
export async function GET(req: NextRequest) {
  ...
  const posResult = await supabase.from('inventory').select('id, name, stock_qty, sale_price', { count: 'exact' }).eq('owner_id', user.id).eq('active', true).limit(5)
  ...
  return NextResponse.json({
    user_id: user.id,
    pos_inventory: { count: posResult.count, sample: posResult.data, error: posResult.error?.message },
