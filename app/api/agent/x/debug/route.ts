...
export async function GET(request: NextRequest) {
  ...
  try {
    const bearerRes = await fetch('https://api.twitter.com/2/tweets/search/recent?query=hello&max_results=10', {
      headers: { Authorization: \`Bearer \${process.env.X_BEARER_TOKEN}\` }
    })
    results.bearer_test = { status: bearerRes.status, body: await bearerRes.json() }
  } catch (e) { results.bearer_test = { error: String(e) } }
  ...
  try {
    ...
    const as_ = process.env.X_ACCESS_TOKEN_SECRET!
    const nonce = Math.random().toString(36).slice(2) + Date.now().toString(36)
    const ts = Math.floor(Date.now()/1000).toString()
    const op: Record<string,string> = { oauth_consumer_key:ck, oauth_nonce:nonce, oauth_signature_method:'HMAC-SHA1', oauth_timestamp:ts, oauth_token:at, oauth_version:'1.0' }
    const sp = Object.keys(op).sort().map(k=>\`\${encodeURIComponent(k)}=\${encodeURIComponent(op[k])}\`).join('&')
    const bs = [url, encodeURIComponent(url), encodeURIComponent(sp)].join('&')
    const sk = \`\${encodeURIComponent(cs)}&\${encodeURIComponent(as_)}\`
  ...
  } catch (e) { results.oauth_test = { error: String(e) } }
