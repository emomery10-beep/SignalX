...
async function runBenScout() {
  ...
  try {
    ...
    const [{ data: recentPosts }, { data: publishedPosts }] = await Promise.all([
      // Was filtering on type='blog_us', which never matches — inserts below
      ...
      supabase.from('agent_content').select('source_query, content').eq('type', 'blog').like('run_id', 'blog_us_%').gte('created_at', thirtyDaysAgo).limit(60),
      supabase.from('agent_content').select('content').eq('type', 'blog').like('run_id', 'blog_us_%').eq('status', 'published').order('created_at', { ascending: false }).limit(30),
    ])
    ...
    log.push(`Searching ${hasTavily ? 'Tavily' : ''}${hasTavily && hasSerper ? ' + ' : ''}${hasSerper ? 'Serper' : ''} for US market data...`)
