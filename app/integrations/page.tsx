  ...
  return (
    ...
    <div style={{ fontFamily: 'DM Sans, system-ui', background: BG, minHeight: '100vh' }}>
      ...
      <nav aria-label="Breadcrumb" style={{ background: SF, borderBottom: `1px solid ${BD}`, padding: '0 clamp(16px,4vw,24px)' }}>
        <ol style={{ listStyle: 'none', margin: 0, padding: '7px 0', display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: TX3 }}>
      ...
      <nav style={{ borderBottom: `1px solid ${BD}`, background: SF, padding: '0 clamp(16px,4vw,24px)', height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <Link href={localePath('/signin', locale)} style={{ fontSize: 11, fontWeight: 600, color: SF, background: ACC, borderRadius: 9999, padding: '7px 18px', textDecoration: 'none' }}>{t('integrations_page.try_free')} →</Link>
      </nav>
      ...
      <section style={{ background: SF, borderBottom: `1px solid ${BD}`, padding: 'clamp(40px,5vw,64px) clamp(16px,4vw,32px)' }}>
        <div style={{ maxWidth: 1060, margin: '0 auto' }}>
          <p style={{ fontSize: 9, fontWeight: 700, color: ACC, letterSpacing: '.16em', textTransform: 'uppercase', marginBottom: 14 }}>{t('integrations_page.eyebrow')}</p>
          ...
          <p style={{ fontSize: 13, color: TX2, maxWidth: 480, lineHeight: 1.7, marginBottom: 0 }}>
      ...
      <section style={{ background: SF, borderBottom: `1px solid ${BD}`, padding: 'clamp(24px,3vw,40px) clamp(16px,4vw,32px)' }}>
        <div style={{ maxWidth: 1060, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 40, alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: '1 1 300px' }}>
            <p style={{ fontSize: 9, fontWeight: 700, color: TX3, letterSpacing: '.14em', textTransform: 'uppercase', marginBottom: 10 }}>eCommerce & accounting</p>
            <p style={{ fontSize: 12, color: TX2, lineHeight: 1.75, margin: 0 }}>
      ...
      <div style={{ maxWidth: 1060, margin: '0 auto', padding: 'clamp(40px,5vw,56px) clamp(16px,4vw,32px)' }}>
        ...
        <div style={{ marginBottom: 56 }}>
          <h2 style={{ fontFamily: 'Sora, system-ui', fontSize: 18, fontWeight: 700, color: TX, marginBottom: 20, letterSpacing: '-.02em' }}>{t('integrations_page.popular_title')}</h2>
            ...
            {INTEGRATIONS.filter(i => i.popular).map(int => (
              <Link key={int.slug} href={localePath('/integrations/' + int.slug, locale)} className="int-card" style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 14, padding: '22px 20px', textDecoration: 'none', display: 'block' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 10, background: int.color + '15', border: `1px solid ${int.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, flexShrink: 0 }}>{int.icon}</div>
                  <div>
                    <div style={{ fontFamily: 'Sora, system-ui', fontSize: 13, fontWeight: 700, color: TX }}>{int.name}</div>
                    <div style={{ fontSize: 9, color: TX3, marginTop: 1 }}>{int.category}</div>
                  </div>
                </div>
                <p style={{ fontSize: 11, color: TX2, lineHeight: 1.55, marginBottom: 12 }}>
                  {(() => { const k = int.slug.replace(/-/g, '_'); const v = t('integrations_page.desc_' + k); return v !== 'integrations_page.desc_' + k ? v : int.description })()}
                </p>
                <span style={{ fontSize: 10, color: ACC, fontWeight: 600 }}>{t('integrations_page.learn_more')} →</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        {INTEGRATION_CATEGORIES.map(cat => {
          ...
          return (
            <div key={cat.slug} style={{ marginBottom: 48 }}>
              <h2 style={{ fontFamily: 'Sora, system-ui', fontSize: 15, fontWeight: 700, color: TX, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
                {cat.icon} {cat.label}
              </h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: 14 }}>
                {catIntegrations.map(int => (
                  <Link key={int.slug} href={localePath('/integrations/' + int.slug, locale)} className="int-card" style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 12, padding: '18px 18px', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 14 }}>
                    <div style={{ width: 40, height: 40, borderRadius: 9, background: int.color + '15', border: `1px solid ${int.color}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, flexShrink: 0 }}>{int.icon}</div>
                    <div>
                      <div style={{ fontFamily: 'Sora, system-ui', fontSize: 12, fontWeight: 700, color: TX, marginBottom: 3 }}>{int.name}</div>
                      <div style={{ fontSize: 10, color: TX3 }}>{int.metrics.slice(0, 2).join(' · ')}</div>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <div style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 16, padding: '32px', textAlign: 'center' }}>
          ...
          <h3 style={{ fontFamily: 'Sora, system-ui', fontSize: 16, fontWeight: 700, color: TX, marginBottom: 8 }}>{t('integrations_page.request_title')}</h3>
          <p style={{ fontSize: 12, color: TX2, marginBottom: 20, maxWidth: 380, margin: '0 auto 20px' }}>{t('integrations_page.request_desc')}</p>
          <Link href={localePath('/help', locale)} style={{ fontSize: 11, fontWeight: 600, color: SF, background: ACC, borderRadius: 8, padding: '10px 22px', textDecoration: 'none', display: 'inline-block' }}>{t('integrations_page.request_cta')}</Link>
        </div>
      </div>
      ...
      <footer style={{ borderTop: `1px solid ${BD}`, padding: '20px clamp(16px,4vw,32px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, background: SF }}>
        <span style={{ fontSize: 10, color: TX3 }}>{t('integrations_page.footer_copy')}</span>
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
          {([['/', 'footer_home'], ['/academy', 'footer_academy'], ['/help', 'footer_help'], ['/developers', 'footer_api'], ['/privacy', 'footer_privacy']] as [string, string][]).map(([href, key]) => (
            <Link key={href} href={localePath(href, locale)} style={{ fontSize: 10, color: TX3, textDecoration: 'none' }}>{t('integrations_page.' + key)}</Link>
          ))}
        </div>
      </footer>
    </div>
