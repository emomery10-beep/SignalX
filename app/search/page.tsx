...
export default function SearchPage() {
  ...
  const results: SearchResult[] = useMemo(() => {
    ...
    academyArticles.forEach(a => {
      ...
      if (score > 0) hits.push({ type: 'academy', title: a.title, description: a.description, href: localePath(`/academy/${a.slug}`, toLocale(lang)), tag: a.category, tagColor: '#d08a59', score })
    })
    ...
    HELP_ARTICLES.forEach(a => {
      ...
      if (score > 0) hits.push({ type: 'help', title: a.title, description: a.description, href: localePath(`/help/${a.slug}`, toLocale(lang)), tag: a.topic, tagColor: '#2980b9', score })
    })
    ...
    blogPosts.forEach((p: any) => {
      ...
      if (score > 0) hits.push({ type: 'blog', title: p.title, description: p.excerpt || p.description || '', href: localePath(`/blog/${p.slug}`, toLocale(lang)), tag: 'Blog', tagColor: '#27ae60', score })
    })
  ...
  return (
    <div style={{ fontFamily: 'DM Sans, system-ui', background: BG, minHeight: '100vh' }}>
      <nav style={{ borderBottom: `1px solid ${BD}`, background: SF, padding: '0 clamp(16px,4vw,24px)', height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <Link href={localePath('/signin', toLocale(lang))} style={{ fontSize: 11, fontWeight: 600, color: SF, background: ACC, borderRadius: 9999, padding: '7px 18px', textDecoration: 'none' }}>Try free →</Link>
      </nav>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: 'clamp(32px,5vw,56px) clamp(16px,4vw,32px)' }}>
          ...
          <>
            {filtered.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '48px 0' }}>
                ...
                <div style={{ fontSize: 11, color: TX3 }}>{tc('common.search_no_results_try')} <Link href={localePath('/academy', toLocale(lang))} style={{ color: ACC }}>{tc('nav.academy')}</Link></div>
              </div>
            ) : (
              <>
        ...
        {query.length < 2 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginTop: 8 }}>
            {([
              { href: localePath('/academy', toLocale(lang)), icon: '🎓', label: 'Browse Academy', sub: `${academyArticles.length}+ guides` },
              { href: localePath('/help', toLocale(lang)),    icon: '❓', label: 'Help Centre',    sub: `${HELP_ARTICLES.length} articles` },
              { href: localePath('/blog', toLocale(lang)),    icon: '📝', label: 'Blog',           sub: 'Latest posts' },
              { href: localePath('/free-tools', toLocale(lang)), icon: '🛠️', label: 'Free Tools', sub: 'Calculators & tools' },
      ...
      <footer style={{ borderTop: `1px solid ${BD}`, padding: '20px clamp(16px,4vw,32px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, background: SF, marginTop: 48 }}>
        ...
        <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
          {([['/', 'Home'], ['/academy', 'Academy'], ['/help', 'Help'], ['/blog', 'Blog']] as [string,string][]).map(([path, label]) => (
            <Link key={path} href={localePath(path, toLocale(lang))} style={{ fontSize: 10, color: TX3, textDecoration: 'none' }}>{label}</Link>
          ))}
        </div>
      </footer>
    </div>
