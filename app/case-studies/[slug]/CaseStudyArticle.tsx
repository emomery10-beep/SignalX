    ...
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', system-ui, sans-serif", color: C.tx }}>
      ...
      <article style={{ maxWidth: 760, margin: "0 auto", padding: "clamp(32px,5vw,56px) clamp(16px,4vw,40px)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 20 }}>
          <span style={{ fontSize: 42 }}>{cs.logo}</span>
          <div>
            <div style={{ fontFamily: "var(--font-sora)", fontSize: 16, fontWeight: 700 }}>{cs.company}</div>
            <div style={{ fontSize: 11, color: C.tx3 }}>{cs.industry} · {cs.location} · {cs.employees}</div>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 32 }}>
          {cs.results.map((r) => (
            <div key={r.metric} style={{ background: C.sf, border: `1.5px solid ${C.b}`, borderRadius: 12, padding: "16px 18px" }}>
              <div style={{ fontSize: 9, color: C.tx3, textTransform: "uppercase" as const, letterSpacing: ".04em", marginBottom: 6 }}>{r.metric}</div>
              <div style={{ fontFamily: "var(--font-sora)", fontSize: 20, fontWeight: 800, color: C.acc, marginBottom: 4 }}>{r.improvement}</div>
              <div style={{ fontSize: 10, color: C.tx3 }}>{r.before} → {r.after}</div>
            </div>
          ))}
        </div>
        ...
        <blockquote style={{ background: C.accBg, border: `1px solid ${C.accBdr}`, borderRadius: 12, padding: "24px 28px", margin: "0 0 36px" }}>
          <p style={{ fontSize: 14, fontStyle: "italic", color: C.tx, lineHeight: 1.65, margin: "0 0 12px" }}>
            "{cs.quote.text}"
          </p>
          <footer style={{ fontSize: 11, color: C.tx2, fontWeight: 600 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 36 }}>
          <div style={{ background: C.el, borderRadius: 8, padding: "8px 14px", fontSize: 11, color: C.tx2 }}>
            <strong style={{ color: C.tx }}>{tc("case_studies.timeline_label")}</strong> {cs.timeline}
          </div>
          {cs.productsUsed.map((p) => (
            <div key={p} style={{ background: C.el, borderRadius: 8, padding: "8px 14px", fontSize: 11, color: C.tx2 }}>
              {p}
            </div>
          ))}
        </div>
        ...
        {cs.sections.map((s) => (
          <div key={s.heading} style={{ marginBottom: 28 }}>
            <h2 style={{ fontFamily: "var(--font-sora)", fontSize: 18, fontWeight: 700, color: C.tx, margin: "0 0 10px" }}>{s.heading}</h2>
            <p style={{ fontSize: 13, color: C.tx2, lineHeight: 1.75, margin: 0 }}>{s.body}</p>
          </div>
        ))}
        ...
        <div style={{ background: "linear-gradient(135deg, #1a1916 0%, #2d2a26 100%)", borderRadius: 14, padding: "36px 32px", textAlign: "center", marginTop: 40 }}>
          <h2 style={{ fontFamily: "var(--font-sora)", fontSize: 20, fontWeight: 700, color: "#fff", margin: "0 0 10px" }}>
        ...
        {related.length > 0 && (
          <div style={{ marginTop: 48 }}>
            <h3 style={{ fontFamily: "var(--font-sora)", fontSize: 16, fontWeight: 700, color: C.tx, margin: "0 0 20px" }}>{tc("case_studies.more_case_studies")}</h3>
              ...
              {related.map((r) => (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                    ...
                    <span style={{ fontFamily: "var(--font-sora)", fontSize: 11, fontWeight: 700 }}>{r.company}</span>
                  </div>
                  <div style={{ fontSize: 12, fontWeight: 600, color: C.tx, lineHeight: 1.4, marginBottom: 8 }}>{r.headline}</div>
                  <span style={{ fontSize: 10, fontWeight: 600, color: C.acc }}>{tc("case_studies.read_short")}</span>
                </Link>
              ))}
            </div>
          </div>
