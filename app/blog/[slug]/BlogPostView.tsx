...
export default function BlogPostView({
  post, prevPost, nextPost, relatedPosts, contextualLinks, academyCrossLinks,
  tocItems, displayPublishDate, lastUpdated, postUrl,
}: {
  post: BlogPost
  prevPost: BlogPost | null
  nextPost: BlogPost | null
  relatedPosts: (BlogPost | null | undefined)[]
  contextualLinks: BlogPost[]
  academyCrossLinks: AcademyArticle[]
  tocItems: { heading: string; id: string }[]
  ...
  const tocItemsWithIds = tocItems

  const displayPublishDateObj = new Date(displayPublishDate)
  const lastUpdatedObj = new Date(lastUpdated)
  ...
  const jsonLdBlogPost = {
    ...
    datePublished: displayPublishDateObj.toISOString(),
    dateModified: lastUpdatedObj.toISOString(),
    author: post.author
    ...
    <div style={{ minHeight: '100vh', background: BG, fontFamily: 'DM Sans, system-ui' }}>
      ...
      <nav style={{ background: SF, borderBottom: `1px solid ${B}`, padding: '0 clamp(16px,4vw,32px)', height: 54, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <Link href={localePath('/blog', toLocale(lang))} className="nav-link" style={{ fontSize: 11, color: TX2, textDecoration: 'none', transition: 'color 120ms' }}>{tc('blog.back_to_articles')}</Link>
          <Link href={localePath('/home', toLocale(lang))} style={{ fontSize: 11, fontWeight: 600, color: ACC, textDecoration: 'none', padding: '6px 14px', background: 'rgba(208,138,89,.1)', borderRadius: 9999 }}>
            Try AskBiz free →
          </Link>
        </div>
      </nav>
      ...
      <article id="article-body" style={{ maxWidth: 720, margin: '0 auto', padding: 'clamp(28px,5vw,52px) clamp(16px,4vw,32px) 80px' }}>
        ...
        <nav aria-label="breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 20, fontSize: 10, color: TX3, flexWrap: 'wrap' }}>
          <Link href={localePath('/', toLocale(lang))} style={{ color: TX3, textDecoration: 'none' }}>{tc('blog.home')}</Link>
          <span>/</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
          ...
          {post.pillar && (
            <Link
              href={localePath(`/blog?cluster=${encodeURIComponent(post.cluster)}&pillar=${encodeURIComponent(post.pillar)}`, toLocale(lang))}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {post.author?.name && <>
            <span style={{ fontSize: 11, fontWeight: 600, color: TX }}>{tc('blog.written_by', { name: post.author.name })}</span>
            <span style={{ fontSize: 11, color: TX3 }}>·</span>
          </>}
          {(() => {
            const diffDays = Math.floor((Date.now() - displayPublishDateObj.getTime()) / 86400000)
            const isRecent = diffDays <= 2
            const label = diffDays === 0 ? 'Today' : diffDays === 1 ? 'Yesterday' : diffDays < 7 ? `${diffDays} days ago` : displayPublishDateObj.toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })
            return (
          ...
          {!post.author && <>
            <span style={{ fontSize: 10, color: TX3 }}>
              Updated {lastUpdatedObj.toLocaleDateString('en-GB', { month: 'short', year: 'numeric' })}
            </span>
            <span style={{ fontSize: 11, color: TX3 }}>·</span>
          </>}
        ...
        <div style={{ background: 'rgba(208,138,89,.06)', border: '1px solid rgba(208,138,89,.25)', borderRadius: 12, padding: '16px 20px', marginBottom: 36 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 9, fontWeight: 700, color: ACC, textTransform: 'uppercase', letterSpacing: '.08em' }}>{tc('blog.key_takeaways')}</span>
          </div>
                  ...
                  <div style={{ background: EV, borderRadius: 10, padding: '14px 18px', margin: '28px 0', border: `1px solid ${B}` }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {contextualLinks.map(p => (
                        <Link key={p.slug} href={localePath(`/blog/${p.slug}`, toLocale(lang))} style={{ fontSize: 11, color: clusterColour.text, textDecoration: 'none', fontWeight: 500, lineHeight: 1.4 }}>
                          → {p.title}
                        </Link>
                      ))}
                    </div>
          ...
          <div style={{ width: 48, height: 48, borderRadius: '50%', background: post.author ? 'linear-gradient(135deg, #6366F1 0%, #818cf8 100%)' : `linear-gradient(135deg, ${ACC} 0%, #e8a870 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, fontSize: 16, fontWeight: 700, color: '#fff', fontFamily: 'Sora, system-ui' }}>
            {post.author ? post.author.name.split(' ').map(n => n[0]).join('') : (
              <svg width="20" height="20" viewBox="0 0 32 32" fill="none">
                <g fill="none" stroke="white" strokeWidth="2.8" strokeLinecap="round" strokeLinejoin="round"><path d="M5 11 V5 H11"/><path d="M21 5 H27 V11"/><path d="M5 21 V27 H11"/><path d="M27 21 V27 H21"/></g><circle cx="16" cy="16" r="2.6" fill="white"/>
              </svg>
            )}
          </div>
          <div>
        ...
        <div style={{ background: TX, borderRadius: 16, padding: 'clamp(20px,4vw,32px)', marginBottom: 48, textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <Link href={localePath(clusterCta?.href || post.cta?.href || '/signin', toLocale(lang))} style={{ display: 'inline-flex', alignItems: 'center', padding: '13px 28px', background: ACC, color: '#fff', borderRadius: 10, fontSize: 12, fontWeight: 700, textDecoration: 'none', boxShadow: '0 2px 12px rgba(208,138,89,.4)' }}>
              {clusterCta?.cta || 'Start free trial →'}
            </Link>
            <Link href={localePath('/pricing', toLocale(lang))} style={{ display: 'inline-flex', alignItems: 'center', padding: '13px 20px', background: 'rgba(255,255,255,.08)', color: 'rgba(255,255,255,.8)', borderRadius: 10, fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
              See pricing
            </Link>
          </div>
        ...
        {(prevPost || nextPost) && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 48 }}>
            {prevPost ? (
              <Link href={localePath(`/blog/${prevPost.slug}`, toLocale(lang))} className="prevnext-card" style={{ display: 'block', padding: '14px 16px', background: SF, border: `1px solid ${B}`, borderRadius: 12, textDecoration: 'none', transition: 'background 120ms' }}>
            ...
            {nextPost && (
              <Link href={localePath(`/blog/${nextPost.slug}`, toLocale(lang))} className="prevnext-card" style={{ display: 'block', padding: '14px 16px', background: SF, border: `1px solid ${B}`, borderRadius: 12, textDecoration: 'none', textAlign: 'right', transition: 'background 120ms' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px,1fr))', gap: 12 }}>
              {relatedPosts.map((r, i) => r && (
                <Link key={i} href={localePath(`/blog/${r.slug}`, toLocale(lang))} className="related-card" style={{ display: 'block', padding: '14px 16px', background: SF, border: `1px solid ${B}`, borderRadius: 12, textDecoration: 'none', transition: 'background 120ms' }}>
          ...
          <div style={{ marginTop: 32 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px,1fr))', gap: 12 }}>
              {academyCrossLinks.map((a, i) => (
                <Link key={i} href={localePath(`/academy/${a.slug}`, toLocale(lang))} className="related-card" style={{ display: 'block', padding: '14px 16px', background: SF, border: `1px solid ${B}`, borderRadius: 12, textDecoration: 'none', transition: 'background 120ms' }}>
