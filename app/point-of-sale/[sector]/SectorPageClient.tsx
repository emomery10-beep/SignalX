    ...
    <div style={{ background: C.bg, minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <nav style={{ background: C.sf, borderBottom: `1px solid ${C.b}`, padding: '0 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
        <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, color: C.tx }}>AskBiz</span>
        </Link>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <Link href="/point-of-sale" style={{ fontSize: 13, color: C.tx2, textDecoration: 'none' }}>← All sectors</Link>
          <a href="https://pos.askbiz.co" target="_blank" rel="noopener noreferrer" style={{ padding: '6px 16px', background: sector.color, color: '#fff', borderRadius: 8, fontSize: 13, fontWeight: 600, textDecoration: 'none' }}>
            Try free
          </a>
        </div>
      </nav>
      ...
      {/* Breadcrumb */}
      <div style={{ background: C.sf, borderBottom: `1px solid ${C.b}`, padding: '10px 24px', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: C.tx3 }}>
      ...
      <section className="hero-section" style={{ background: `linear-gradient(135deg, ${sector.color}08 0%, ${sector.color}04 100%)`, borderBottom: `1px solid ${sector.color}20`, padding: '56px 24px 64px', textAlign: 'center' }}>
        <div style={{ maxWidth: 680, margin: '0 auto' }}>
          <div className="fade-up" style={{ fontSize: 56, marginBottom: 12 }}>{sector.icon}</div>
          <div className="fade-up" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: `${sector.color}12`, border: `1px solid ${sector.color}30`, borderRadius: 20, padding: '4px 14px', marginBottom: 16 }}>
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: sector.color }}>AskBiz PoS — {sector.label}</span>
          </div>
          <h1 className="fade-up hero-title" style={{ fontSize: 36, fontWeight: 800, color: C.tx, lineHeight: 1.2, margin: '0 0 16px', letterSpacing: '-0.02em' }}>{sector.tagline}</h1>
          <p className="fade-up" style={{ fontSize: 17, color: C.tx2, lineHeight: 1.65, margin: '0 0 32px' }}>{sector.hero}</p>
          <div className="fade-up" style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="https://pos.askbiz.co" target="_blank" rel="noopener noreferrer" style={{ padding: '12px 28px', background: sector.color, color: '#fff', borderRadius: 10, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: `0 4px 14px ${sector.color}40` }}>
              Start free — {posPrice}/seat/mo
            </a>
            <Link href="/point-of-sale#features" style={{ padding: '12px 24px', background: C.sf, border: `1.5px solid ${C.b2}`, color: C.tx, borderRadius: 10, fontSize: 14, fontWeight: 500, textDecoration: 'none' }}>
              See all features
            </Link>
          </div>
        </div>
        ...
        <div className="stats-grid" style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0 }}>
          {sector.stats.map((stat, i) => (
            <div key={i} style={{ textAlign: 'center', padding: '12px 8px', borderRight: i < sector.stats.length - 1 ? '1px solid rgba(255,255,255,.2)' : 'none' }}>
              <div style={{ fontSize: 22, fontWeight: 800, color: '#fff', letterSpacing: '-0.02em' }}>{stat.value}</div>
              <div style={{ fontSize: 11, color: 'rgba(255,255,255,.75)', marginTop: 3 }}>{stat.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '64px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: sector.color, textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 8 }}>Built for {sector.label}</div>
            <h2 style={{ fontSize: 28, fontWeight: 800, color: C.tx, margin: '0 0 12px', letterSpacing: '-0.02em' }}>Every feature your {sector.label.toLowerCase()} needs</h2>
            <p style={{ fontSize: 15, color: C.tx2, maxWidth: 520, margin: '0 auto' }}>{sector.desc}</p>
          </div>
          <div className="features-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>
            {sector.features.map((f, i) => (
              <div key={i} className="card-hover" style={{ background: C.sf, border: `1px solid ${C.b}`, borderRadius: 12, padding: '20px 22px' }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 700, color: C.tx, marginBottom: 6 }}>{f.title}</div>
                <div style={{ fontSize: 13, color: C.tx2, lineHeight: 1.6 }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: C.sf, borderTop: `1px solid ${C.b}`, borderBottom: `1px solid ${C.b}`, padding: '64px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: sector.color, textTransform: 'uppercase', letterSpacing: '.1em', marginBottom: 8 }}>Workflow</div>
            <h2 style={{ fontSize: 28, fontWeight: 800, color: C.tx, margin: '0 0 12px', letterSpacing: '-0.02em' }}>How it works</h2>
          </div>
            ...
            {sector.workflow.map((step, i) => (
              <div key={i} style={{ position: 'relative' }}>
                ...
                <div style={{ width: 36, height: 36, borderRadius: '50%', background: `${sector.color}15`, border: `2px solid ${sector.color}40`, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: sector.color }}>{step.step}</span>
                </div>
                <div style={{ fontSize: 14, fontWeight: 700, color: C.tx, marginBottom: 6 }}>{step.title}</div>
                <div style={{ fontSize: 13, color: C.tx2, lineHeight: 1.6 }}>{step.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: `linear-gradient(135deg, ${sector.color}10, ${sector.color}05)`, border: `1px solid ${sector.color}20`, margin: '0 24px 64px', borderRadius: 16, padding: '48px 32px', maxWidth: 900, marginLeft: 'auto', marginRight: 'auto' }}>
        <div style={{ textAlign: 'center', maxWidth: 560, margin: '0 auto' }}>
          <div style={{ fontSize: 36, marginBottom: 12 }}>{sector.icon}</div>
          <h2 style={{ fontSize: 26, fontWeight: 800, color: C.tx, margin: '0 0 12px', letterSpacing: '-0.02em' }}>Start your {sector.label.toLowerCase()} PoS today</h2>
          <p style={{ fontSize: 15, color: C.tx2, lineHeight: 1.65, margin: '0 0 28px' }}>
          ...
          <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="https://pos.askbiz.co" target="_blank" rel="noopener noreferrer" style={{ padding: '13px 30px', background: sector.color, color: '#fff', borderRadius: 10, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: `0 4px 14px ${sector.color}35` }}>
              Start free →
            </a>
            <Link href="/point-of-sale" style={{ padding: '13px 24px', background: C.sf, border: `1.5px solid ${C.b2}`, color: C.tx, borderRadius: 10, fontSize: 14, fontWeight: 500, textDecoration: 'none' }}>
              Compare all plans
            </Link>
          </div>
          <p style={{ fontSize: 11, color: C.tx3, marginTop: 14 }}>Included in every plan from {growthPrice}/mo · No credit card required to start</p>
        </div>
      </section>
      ...
      <section style={{ padding: '0 24px 80px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: C.tx2, marginBottom: 6 }}>More sectors</div>
            <h3 style={{ fontSize: 22, fontWeight: 700, color: C.tx, margin: 0 }}>AskBiz PoS works for every type of business</h3>
          </div>
            ...
            {otherSectors.map(s => (
              <Link key={s.id} href={`/point-of-sale/${s.id}`} style={{ textDecoration: 'none' }}>
                <div className="card-hover sector-link" style={{ background: C.sf, border: `1px solid ${C.b}`, borderRadius: 12, padding: '16px 14px', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, color: C.tx, marginBottom: 4 }}>{s.label}</div>
                  <div style={{ fontSize: 11, color: C.tx3, lineHeight: 1.5 }}>{s.tagline}</div>
                </div>
              </Link>
            ))}
          </div>
