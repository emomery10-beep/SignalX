  ...
  return (
    <div style={{ fontFamily: 'var(--font-dm, system-ui)', background: '#f9f8f6', minHeight: '100vh' }}>
      <nav style={{ borderBottom: '1px solid #e8e6e1', background: '#fff', padding: '0 clamp(16px,4vw,24px)', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 50 }}>
        ...
        <Link href={localePath('/signin', lang)} style={{ fontSize: 13, fontWeight: 600, color: '#fff', background: '#d08a59', borderRadius: 9999, padding: '7px 16px', textDecoration: 'none' }}>Try free →</Link>
      </nav>
      <main style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(40px,6vw,64px) clamp(16px,4vw,24px)' }}>
        ...
        <div style={{ marginTop: 48, padding: '24px', borderRadius: 16, background: 'rgba(208,138,89,.06)', border: '1px solid rgba(208,138,89,.2)', textAlign: 'center' }}>
          ...
          <Link href={localePath('/signin', lang)} style={{ fontSize: 14, fontWeight: 600, color: '#fff', background: '#d08a59', borderRadius: 9999, padding: '10px 24px', textDecoration: 'none', display: 'inline-block' }}>Try free →</Link>
        </div>
      </main>
    </div>
  )
