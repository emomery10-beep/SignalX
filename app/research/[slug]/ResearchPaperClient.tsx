  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', system-ui, sans-serif", color: C.tx }}>
      ...
      <article style={{ maxWidth: 760, margin: "0 auto", padding: "clamp(28px,5vw,52px) clamp(16px,4vw,40px) clamp(48px,7vw,80px)" }}>
        <Link href={localePath("/research", toLocale(lang))} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, color: C.tx3, textDecoration: "none", marginBottom: 26 }}>
          ← Research
        </Link>

        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: ".06em", textTransform: "uppercase", color: C.acc, background: C.accBg, border: `1px solid ${C.accBdr}`, borderRadius: 9999, padding: "4px 11px" }}>{paper.type}</span>
          <span style={{ fontSize: 11, color: C.tx3 }}>{paper.date}</span>
          {paper.length && <span style={{ fontSize: 11, color: C.tx3 }}>· {paper.length}</span>}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 11, paddingBottom: 26, marginBottom: 30, borderBottom: `1px solid ${C.b}` }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: C.tx }}>{AUTHOR.name}</div>
            <div style={{ fontSize: 10.5, color: C.tx3 }}>{AUTHOR.role}</div>
          </div>
        </div>

        <section style={{ marginBottom: 34 }}>
          <h2 style={{ fontFamily: "var(--font-sora)", fontSize: 11, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: C.tx3, margin: "0 0 14px" }}>Abstract</h2>
          <p style={{ fontSize: 14.5, lineHeight: 1.75, color: C.tx, margin: 0 }}>{paper.abstract}</p>
        </section>

        <section style={{ marginBottom: 36 }}>
          <h2 style={{ fontFamily: "var(--font-sora)", fontSize: 11, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", color: C.tx3, margin: "0 0 16px" }}>Key findings</h2>
          <ol style={{ listStyle: "none", counterReset: "kf", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: 16 }}>
            {paper.keyFindings.map((f, i) => (
              <li key={i} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                <span aria-hidden="true" style={{ flexShrink: 0, width: 26, height: 26, borderRadius: "50%", background: C.accBg, color: C.acc, border: `1px solid ${C.accBdr}`, fontSize: 11, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }}>{i + 1}</span>
                <p style={{ fontSize: 13.5, lineHeight: 1.7, color: C.tx2, margin: 0 }}>{f}</p>
              </li>
            ))}
          </ol>
        </section>
        ...
        <div style={{ background: C.sf, border: `1px solid ${C.b}`, borderRadius: 16, padding: "clamp(20px,3vw,26px)", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
          <div>
            <div style={{ fontFamily: "var(--font-sora)", fontSize: 13, fontWeight: 700, color: C.tx, marginBottom: 3 }}>Read the full paper</div>
            <div style={{ fontSize: 11, color: C.tx3 }}>PDF · {paper.length} · free to download</div>
          </div>
          <a href={paper.file} target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 24px", borderRadius: 9999, background: C.acc, color: "#fff", fontSize: 12, fontWeight: 600, textDecoration: "none", transition: "background .15s ease" }}
