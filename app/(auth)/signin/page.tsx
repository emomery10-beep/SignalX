    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={handleGoogle} disabled={loading}
          style={{ width: '100%', padding: '11px', borderRadius: 10, border: '1px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 14, fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 16px', gap: 9, marginBottom: 16, transition: 'all 150ms' }}>
        ...
        {/* Primary CTA */}
        <button onClick={handleEmailAuth} disabled={loading || !email || !password || (mode === 'signup' && (!agreedToTerms || !agreedToData || !agreedToAge))}
        ...
        <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
            <input type="checkbox" checked={agreedToTerms} onChange={e => setAgreedToTerms(e.target.checked)}
              style={{ marginTop: 2, width: 16, height: 16, flexShrink: 0, cursor: 'pointer', accentColor: 'var(--acc)' }}/>
            <span style={{ fontSize: 12, color: 'var(--tx3)', lineHeight: 1.5 }}>
              I agree to the <Link href="/terms" style={{ color: 'var(--acc)', textDecoration: 'none' }}>Terms of Service</Link> and <Link href="/privacy" style={{ color: 'var(--acc)', textDecoration: 'none' }}>Privacy Policy</Link>
            </span>
          </label>
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
            <input type="checkbox" checked={agreedToData} onChange={e => setAgreedToData(e.target.checked)}
              style={{ marginTop: 2, width: 16, height: 16, flexShrink: 0, cursor: 'pointer', accentColor: 'var(--acc)' }}/>
            <span style={{ fontSize: 12, color: 'var(--tx3)', lineHeight: 1.5 }}>
              I consent to AskBiz processing my business data to provide AI-powered insights. I can withdraw consent at any time in Settings. <Link href="/privacy#data" style={{ color: 'var(--acc)', textDecoration: 'none' }}>Learn more</Link>
            </span>
          </label>
          <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
            <input type="checkbox" checked={agreedToAge} onChange={e => setAgreedToAge(e.target.checked)}
              style={{ marginTop: 2, width: 16, height: 16, flexShrink: 0, cursor: 'pointer', accentColor: 'var(--acc)' }}/>
            <span style={{ fontSize: 12, color: 'var(--tx3)', lineHeight: 1.5 }}>
              I confirm I am at least 13 years old. Users under 16 in the EU or under 18 in some jurisdictions require parental consent before using AskBiz.
            </span>
          </label>
        </div>
      ) : (
