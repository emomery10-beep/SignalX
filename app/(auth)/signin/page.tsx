...
export default function AuthPage() {
  ...
  const inp: React.CSSProperties = {
    ...
    color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(11px,1.2vw,12px)',
    outline: 'none', boxSizing: 'border-box',
  }
  const lbl: React.CSSProperties = {
    display: 'block', minHeight: 0, fontFamily: 'inherit', fontSize: 'clamp(10px,1.1vw,11px)', fontWeight: 500,
    ...
    <div style={{
      ...
      {searchParams.get('ref') === 'shopify' && searchParams.get('shop') && (
        <div className="animate-fade-down" style={{
          ...
          <div>
            <div style={{ fontSize: 'clamp(10px,1.1vw,11px)', fontWeight: 600, color: '#3a6600', marginBottom: 2 }}>
              {tc('auth.shopify_connected', { shop: searchParams.get('shop') || '' })}
            </div>
            <div style={{ fontSize: 'clamp(9px,1vw,10px)', color: '#5A8A00', lineHeight: 1.4 }}>
      ...
      <div
        ...
        {passkeyNudge.show ? (
          <div className="animate-fade-up" style={{ textAlign: 'center', padding: '5px 0' }}>
            ...
            <p style={{ fontSize: 'clamp(10px,1.2vw,11px)', color: 'var(--tx2)', marginBottom: 14, lineHeight: 1.5 }}>{tc('auth.passkey_nudge_body')}</p>
            <button onClick={addPasskeyFromNudge} disabled={nudgeBusy}
              style={{ width: '100%', minHeight: 0, padding: '8px', borderRadius: 9999, border: 'none', background: 'var(--acc)', color: '#fff', fontFamily: 'var(--font-sora, Sora)', fontSize: 'clamp(11px,1.2vw,12px)', fontWeight: 600, cursor: nudgeBusy ? 'wait' : 'pointer', opacity: nudgeBusy ? .7 : 1, marginBottom: 7 }}>
              {nudgeBusy ? tc('auth.please_wait') : tc('auth.passkey_nudge_cta')}
            </button>
            <button onClick={dismissPasskeyNudge} disabled={nudgeBusy}
              style={{ width: '100%', minHeight: 0, padding: '7px', borderRadius: 9999, border: '1px solid var(--b2)', background: 'transparent', color: 'var(--tx2)', fontFamily: 'inherit', fontSize: 'clamp(10px,1.1vw,11px)', cursor: nudgeBusy ? 'not-allowed' : 'pointer', marginBottom: 10 }}>
            ...
            <label style={{ minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, cursor: 'pointer' }}>
              ...
              <span style={{ fontSize: 'clamp(9px,1vw,10px)', color: 'var(--tx3)' }}>{tc('auth.passkey_nudge_dont_remind')}</span>
            </label>
          </div>
        ) : (<>
          ...
          {(['signin', 'signup'] as Mode[]).map(m => (
            <button key={m} onClick={() => { setMode(m); setPin(''); setPinConfirm(''); setError(''); setSuccess('') }}
              style={{
                ...
                fontFamily: 'inherit', fontSize: 'clamp(10px,1.2vw,11px)', fontWeight: 600,
        ...
        <p className="animate-fade-up stagger-2" style={{ fontSize: 'clamp(11px,1.3vw,12px)', color: 'var(--tx2)', marginBottom: 9, textAlign: 'center', lineHeight: 1.4 }}>
        ...
        <button className="animate-fade-up stagger-3 no-tap-target" onClick={() => triggerAuth('google')} disabled={loading}
          style={{ width: '100%', padding: '0 12px', height: 28, borderRadius: 7, border: '1px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(11px,1.2vw,12px)', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 5 }}>
        ...
        <button className="animate-fade-up stagger-4" onClick={() => triggerAuth('azure')} disabled={loading}
          style={{ width: '100%', minHeight: 0, padding: '0 12px', height: 28, borderRadius: 7, border: '1px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(11px,1.2vw,12px)', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 5 }}>
        ...
        {mode === 'signin' && (
          <button className="animate-fade-up stagger-5" onClick={handlePasskeySignIn} disabled={loading}
            style={{ width: '100%', minHeight: 0, padding: '0 12px', height: 28, borderRadius: 7, border: '1px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(11px,1.2vw,12px)', fontWeight: 500, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, marginBottom: 7 }}>
        ...
        <div className="animate-fade-up stagger-5" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
          ...
          <span style={{ fontSize: 'clamp(9px,1vw,10px)', color: 'var(--tx3)' }}>{tc('auth.or')}</span>
          <div style={{ flex: 1, height: 1, background: 'var(--b)' }}></div>
        </div>
        ...
        {mode === 'signup' && (
          <>
            <p className="animate-fade-up" style={{ fontSize: 'clamp(10px,1.1vw,11px)', fontWeight: 600, color: 'var(--tx)', marginBottom: 4 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: mode === 'signup' ? 'repeat(2, 1fr)' : '1fr', gap: 6, marginBottom: 5 }}>
            <div>
              ...
              <input id="pin" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, '').slice(0, 4))}
                ...
                dir="ltr" style={{ ...inp, textAlign: 'center', letterSpacing: pin ? '.5em' : 'normal', fontSize: 'clamp(14px,1.6vw,16px)' }}
                onKeyDown={e => e.key === 'Enter' && handlePhoneAuth()}/>
            </div>
            {mode === 'signup' && (
              <div>
                ...
                <input id="pinConfirm" value={pinConfirm} onChange={e => setPinConfirm(e.target.value.replace(/\D/g, '').slice(0, 4))}
                  ...
                  dir="ltr" style={{ ...inp, textAlign: 'center', letterSpacing: pinConfirm ? '.5em' : 'normal', fontSize: 'clamp(14px,1.6vw,16px)' }}
                  onKeyDown={e => e.key === 'Enter' && handlePhoneAuth()}/>
              </div>
            )}
          </div>
          <p className="animate-fade-up" style={{ fontSize: 'clamp(9px,1vw,10px)', color: 'var(--tx3)', marginBottom: 7, lineHeight: 1.4 }}>
            {tc('auth.phone_hint')}
          </p>
        </>)}
        ...
        {mode === 'signup' && (
          <label htmlFor="consent" className="animate-fade-up" style={{ minHeight: 0, display: 'flex', alignItems: 'flex-start', gap: 6, marginBottom: 7, cursor: 'pointer' }}>
            ...
            />
            <span style={{ fontSize: 'clamp(9px,1vw,10px)', color: 'var(--tx2)', lineHeight: 1.4 }}>
        ...
        {/* Error / Success */}
        {error && <div role="alert" aria-live="polite" className="msg-in" style={{ padding: '6px 9px', borderRadius: 7, background: 'rgba(244,128,128,.1)', border: '1px solid rgba(244,128,128,.3)', fontSize: 'clamp(10px,1.1vw,11px)', color: '#b91c1c', marginBottom: 6 }}>{error}</div>}
        {success && <div role="status" aria-live="polite" className="msg-in" style={{ padding: '6px 9px', borderRadius: 7, background: 'rgba(34,197,94,.1)', border: '1px solid rgba(34,197,94,.3)', fontSize: 'clamp(10px,1.1vw,11px)', color: '#15803d', marginBottom: 6 }}>✓ {success}</div>}
        ...
        {(() => {
          ...
          return (
            <button onClick={onClick} disabled={disabled}
              style={{ width: '100%', minHeight: 0, padding: '7px', borderRadius: 9999, border: 'none', background: 'var(--acc)', color: '#fff', fontFamily: 'var(--font-sora, Sora)', fontSize: 'clamp(12px,1.4vw,13px)', fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? .6 : 1, marginBottom: 5 }}>
              {loading ? (
        ...
        {mode === 'signup' && (
          <p style={{ fontSize: 'clamp(9px,0.9vw,9px)', color: 'var(--tx3)', textAlign: 'center', lineHeight: 1.5, marginTop: 3 }}>
            {tc('auth.age_confirm')}{' '}
        ...
        {mode === 'signup' && (
          <p style={{ fontSize: 'clamp(10px,1.1vw,11px)', color: 'var(--tx3)', textAlign: 'center', marginTop: 5 }}>
            Already have an account?{' '}
            <button onClick={() => setMode('signin')} style={{ minHeight: 0, background: 'none', border: 'none', color: 'var(--acc)', fontFamily: 'inherit', fontSize: 'clamp(10px,1.1vw,11px)', fontWeight: 600, cursor: 'pointer', padding: 0, display: 'inline' }}>
              Sign in
            </button>
          </p>
        )}
        ...
        <button onClick={handleMagicLink} disabled={loading || !email}
          style={{ width: '100%', minHeight: 0, padding: '6px', borderRadius: 9999, border: '1px solid var(--b2)', background: 'transparent', color: 'var(--tx2)', fontFamily: 'inherit', fontSize: 'clamp(10px,1.2vw,11px)', cursor: loading || !email ? 'not-allowed' : 'pointer', opacity: loading || !email ? .6 : 1 }}>
      ...
      <p style={{ fontSize: 'clamp(9px,1vw,10px)', color: 'var(--tx3)', marginTop: 10, textAlign: 'center', flexShrink: 0 }}>
