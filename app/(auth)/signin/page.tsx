...
export default function AuthPage() {
  ...
  const inp: React.CSSProperties = {
    width: '100%', minHeight: 0, padding: '13px 14px', borderRadius: 10,
    border: '2px solid var(--b2)', background: 'var(--ev)',
    color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(17px,1.8vw,18px)',
    outline: 'none', boxSizing: 'border-box',
  }
  const lbl: React.CSSProperties = {
    display: 'block', minHeight: 0, fontFamily: 'inherit', fontSize: 'clamp(15px,1.4vw,16px)', fontWeight: 600,
  ...
  return (
    <div style={{
      ...
      <div
        ...
        <div style={{
          ...
          {(['signin', 'signup'] as Mode[]).map(m => (
            <button key={m} onClick={() => { setMode(m); setPin(''); setPinConfirm(''); setError(''); setSuccess('') }}
              style={{
                position: 'relative', zIndex: 1, minHeight: 48,
                flex: 1, padding: '12px', borderRadius: 9, border: 'none',
                background: 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                color: mode === m ? '#fff' : 'var(--tx2)',
                fontFamily: 'inherit', fontSize: 'clamp(17px,1.6vw,19px)', fontWeight: 700,
        ...
        <h1 className="animate-fade-up stagger-2" style={{ fontFamily: 'var(--font-sora, Sora)', fontSize: 'clamp(20px,2.6vw,24px)', fontWeight: 700, marginBottom: 4, textAlign: 'center', letterSpacing: '-.02em' }}>
          {mode === 'signin' ? tc('auth.welcome_back') : tc('auth.get_started')}
        </h1>
        <p className="animate-fade-up stagger-2" style={{ fontSize: 'clamp(15px,1.6vw,16px)', color: 'var(--tx2)', marginBottom: 14, textAlign: 'center', lineHeight: 1.45 }}>
        ...
        <button className="animate-fade-up stagger-3" onClick={() => triggerAuth('google')} disabled={loading}
          style={{ width: '100%', padding: '0 12px', height: 52, borderRadius: 12, border: '2px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(17px,1.6vw,18px)', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginBottom: 8 }}>
        ...
        <button className="animate-fade-up stagger-4" onClick={() => triggerAuth('azure')} disabled={loading}
          style={{ width: '100%', minHeight: 0, padding: '0 12px', height: 52, borderRadius: 12, border: '2px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(17px,1.6vw,18px)', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginBottom: 8 }}>
          <svg width="18" height="18" viewBox="0 0 23 23" fill="none"><path fill="#f25022" d="M1 1h10v10H1z"/><path fill="#00a4ef" d="M1 12h10v10H1z"/><path fill="#7fba00" d="M12 1h10v10H12z"/><path fill="#ffb900" d="M12 12h10v10H12z"/></svg>
          {tc('auth.continue_microsoft')}
        </button>
        {mode === 'signin' && (
          <button className="animate-fade-up stagger-5" onClick={handlePasskeySignIn} disabled={loading}
            style={{ width: '100%', minHeight: 0, padding: '0 12px', height: 52, borderRadius: 12, border: '2px solid var(--b2)', background: 'var(--sf)', color: 'var(--tx)', fontFamily: 'inherit', fontSize: 'clamp(17px,1.6vw,18px)', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, marginBottom: 10 }}>
        ...
        {/* Divider */}
        <div className="animate-fade-up stagger-5" style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
          <div style={{ flex: 1, height: 1, background: 'var(--b)' }}></div>
          <span style={{ fontSize: 'clamp(14px,1.2vw,15px)', color: 'var(--tx3)', fontWeight: 500 }}>{tc('auth.or')}</span>
          <div style={{ flex: 1, height: 1, background: 'var(--b)' }}></div>
        </div>

        {/* Email / Phone method toggle */}
        <div className="animate-fade-up stagger-6" style={{ position: 'relative', display: 'flex', background: 'var(--ev)', borderRadius: 11, padding: 3, marginBottom: 10 }}>
          <div aria-hidden style={{
          ...
          {(['phone', 'email'] as Method[]).map(m => (
            <button key={m} onClick={() => { setMethod(m); setError(''); setSuccess('') }}
              style={{
                position: 'relative', zIndex: 1, minHeight: 46,
                flex: 1, padding: '10px', borderRadius: 8, border: 'none',
                background: 'transparent',
                color: method === m ? 'var(--tx)' : 'var(--tx2)',
                fontFamily: 'inherit', fontSize: 'clamp(16px,1.5vw,18px)', fontWeight: 700,
                cursor: 'pointer', transition: 'color .2s var(--ease-out)',
                display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
              }}>
              {m === 'email' ? (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-10 6L2 7"/></svg>
              ) : (
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="7" y="2" width="10" height="20" rx="2"/><path d="M11 18h2"/></svg>
              )}
          ...
          <div className="animate-fade-up" style={{ marginBottom: 5 }}>
            ...
            <div style={{ display: 'flex', gap: 5 }} dir="ltr">
              <select value={phoneCountry} onChange={e => setPhoneCountry(e.target.value)} aria-label={tc('auth.method_phone')}
                style={{ ...inp, width: 92, flexShrink: 0, cursor: 'pointer', appearance: 'none' }}>
                {COUNTRY_DIAL.map(c => (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: mode === 'signup' ? 'repeat(2, 1fr)' : '1fr', gap: 6, marginBottom: 5 }}>
            <div>
              ...
              <input id="pin" value={pin} onChange={e => setPin(e.target.value.replace(/\D/g, '').slice(0, 4))}
                ...
                dir="ltr" style={{ ...inp, textAlign: 'center', letterSpacing: pin ? '.6em' : 'normal', fontSize: 'clamp(20px,2.2vw,22px)', fontWeight: 700 }}
                onKeyDown={e => e.key === 'Enter' && handlePhoneAuth()}/>
            </div>
            {mode === 'signup' && (
              <div>
                ...
                <input id="pinConfirm" value={pinConfirm} onChange={e => setPinConfirm(e.target.value.replace(/\D/g, '').slice(0, 4))}
                  ...
                  dir="ltr" style={{ ...inp, textAlign: 'center', letterSpacing: pinConfirm ? '.6em' : 'normal', fontSize: 'clamp(20px,2.2vw,22px)', fontWeight: 700 }}
                  onKeyDown={e => e.key === 'Enter' && handlePhoneAuth()}/>
              </div>
            )}
          </div>
        ...
        {(() => {
          ...
          return (
            <button onClick={onClick} disabled={disabled}
              style={{ width: '100%', minHeight: 56, padding: '14px', borderRadius: 9999, border: 'none', background: 'var(--acc)', color: '#fff', fontFamily: 'var(--font-sora, Sora)', fontSize: 'clamp(18px,1.9vw,20px)', fontWeight: 700, cursor: disabled ? 'not-allowed' : 'pointer', opacity: disabled ? .6 : 1, marginBottom: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {loading ? (
                <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                  <svg style={{ animation: 'spin .7s linear infinite' }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M12 2v4M12 18v4M4.93 4.93l2.83 2.83M16.24 16.24l2.83 2.83M2 12h4M18 12h4M4.93 19.07l2.83-2.83M16.24 7.76l2.83-2.83"/></svg>
                  {tc('auth.please_wait')}
                </span>
              ) : label}
            </button>
