  ...
  return (
    <div style={{ fontFamily: 'var(--font-dm, system-ui)', background: '#f9f8f6', minHeight: '100vh' }}>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(40px,6vw,64px) clamp(16px,4vw,24px)' }}>
        ...
        <p style={{ fontSize: 14, color: '#6b6760', lineHeight: 1.7, marginBottom: 36, maxWidth: 560 }}>Every piece of financial jargon your accountant, investor, or analyst uses — translated into language you can actually use to run your business.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 }}>
          {entries.map(e => (
            <Link key={e.slug} href={localePath(`/translate/${e.slug}`, lang)} style={{ padding: '18px 20px', borderRadius: 14, border: '1px solid #e8e6e1', background: '#fff', textDecoration: 'none', display: 'block' }}>
              <div style={{ fontFamily: 'var(--font-sora, system-ui)', fontSize: 12, fontWeight: 700, color: '#8c6fe0', marginBottom: 4 }}>{e.term}</div>
