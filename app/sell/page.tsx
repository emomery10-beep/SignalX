  ...
  useEffect(() => {
    ...
    if (session) {
      try {
        ...
        if (s.role === 'inventory') { router.push('/inventory'); return }
        setStaff(s)
  ...
  if (screen === 'login') return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f9f8f6', padding: 24 }}>
      <div style={{ width: '100%', maxWidth: 360, textAlign: 'center' }}>
        <div style={{ fontSize: 28, fontWeight: 900, color: '#1a1916', marginBottom: 4 }}>AskBiz POS</div>
        <div style={{ fontSize: 14, color: '#6b6760', marginBottom: 32 }}>Staff sign-in</div>

        <div style={{ background: '#fff', borderRadius: 20, padding: '32px 24px', border: '1px solid #e5e2dc' }}>
          {loginStep === 'email' ? (
            <>
              <div style={{ fontSize: 15, fontWeight: 600, color: '#1a1916', marginBottom: 16, textAlign: 'left' }}>Enter your email</div>
              <input
                type="email"
                autoFocus
            ...
            <>
              ...
              <div style={{ fontSize: 13, color: '#6b6760', marginBottom: 16, textAlign: 'left' }}>Enter your PIN</div>
              <input
                type="password"
                autoFocus
                inputMode="numeric"
        ...
        <div style={{ marginTop: 24, fontSize: 12, color: '#a39e97' }}>
          Ask your manager if you don't have an account
        </div>
      </div>
    </div>
  )
  ...
  if (screen === 'home') return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#f9f8f6' }}>
      <div style={{ padding: '20px 20px 8px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <button onClick={() => { localStorage.removeItem('pos_staff'); setStaff(null); setScreen('login'); setLoginStep('email'); setLoginEmail(''); setLoginPin('') }} style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid #e5e2dc', background: 'transparent', fontSize: 12, cursor: 'pointer', color: '#6b6760' }}>Sign out</button>
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', gap: 16 }}>
        ...
        {cart.length > 0 && (
          ...
          >
            View Cart ({cart.length}) · {sym}{cartTotal.toFixed(2)}
          </button>
        )}
      </div>
    </div>
  ...
  if (screen === 'scan') return (
    <div style={{ minHeight: '100vh', background: '#000', display: 'flex', flexDirection: 'column' }}>
      <div style={{ padding: '20px', display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <div style={{ color: '#fff', fontSize: 16, fontWeight: 600 }}>Scan product</div>
      </div>
      ...
      {scanning ? (
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16, padding: '24px' }}>
          ...
          <div style={{ color: '#fff', fontSize: 16, fontWeight: 600 }}>Identifying product...</div>
