  ...
  return (
    <div style={{ fontFamily: 'DM Sans, system-ui', background: BG, minHeight: '100vh' }}>
      ...
      <section style={{ background: `linear-gradient(135deg, ${u.color}18 0%, ${BG} 60%)`, padding: 'clamp(56px,7vw,96px) clamp(16px,4vw,32px)', textAlign: 'center', borderBottom: `1px solid ${BD}` }}>
        <div style={{ fontSize: 50, marginBottom: 16 }}>{u.icon}</div>
        <div style={{ fontSize: 10, fontWeight: 700, color: u.color, textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 14 }}>AskBiz for {u.title}</div>
        ...
        <p style={{ fontSize: 15, color: TX2, maxWidth: 600, margin: '0 auto 36px', lineHeight: 1.7 }}>
          {u.subheadline}
        </p>
        <Link href={localePath('/signin', lang)} style={{ fontSize: 13, fontWeight: 700, color: SF, background: u.color, borderRadius: 10, padding: '14px 32px', textDecoration: 'none', display: 'inline-block' }}>
          {u.cta} →
        </Link>
      </section>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: 'clamp(40px,5vw,72px) clamp(16px,4vw,32px)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 18, marginBottom: 72 }}>
          {u.metrics.map((m, i) => (
            <div key={i} style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 14, padding: '24px 22px', textAlign: 'center' }}>
              <div style={{ fontFamily: 'Sora, system-ui', fontSize: 30, fontWeight: 700, color: u.color, marginBottom: 6 }}>{m.value}</div>
              <div style={{ fontSize: 12, fontWeight: 600, color: TX, marginBottom: 4 }}>{m.label}</div>
              <div style={{ fontSize: 10, color: TX3, lineHeight: 1.5 }}>{m.description}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 48, marginBottom: 72, alignItems: 'start' }}>
          <div>
            <h2 style={{ fontFamily: 'Sora, system-ui', fontSize: 20, fontWeight: 700, color: TX, marginBottom: 20, letterSpacing: '-.02em' }}>
              Sound familiar?
            </h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {u.painPoints.map((p, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '14px 16px', background: SF, border: `1px solid ${BD}`, borderRadius: 10 }}>
                  <span style={{ color: '#e74c3c', fontSize: 14, flexShrink: 0, marginTop: 1 }}>✕</span>
                  <span style={{ fontSize: 12, color: TX2, lineHeight: 1.5 }}>{p}</span>
                </div>
              ))}
            </div>
          </div>
          <div>
            ...
            <div style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 14, padding: '24px', lineHeight: 1.8 }}>
              <p style={{ fontSize: 12, color: TX2, margin: '0 0 16px' }}>{u.description}</p>
              <Link href={localePath('/signin', lang)} style={{ fontSize: 11, fontWeight: 600, color: SF, background: u.color, borderRadius: 8, padding: '9px 20px', textDecoration: 'none', display: 'inline-block' }}>
        ...
        <div style={{ marginBottom: 72 }}>
          <h2 style={{ fontFamily: 'Sora, system-ui', fontSize: 20, fontWeight: 700, color: TX, marginBottom: 28, letterSpacing: '-.02em', textAlign: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 18 }}>
            {u.features.map((f, i) => (
              <div key={i} style={{ background: SF, border: `1px solid ${BD}`, borderRadius: 14, padding: '22px 20px' }}>
                ...
                <div style={{ fontFamily: 'Sora, system-ui', fontSize: 13, fontWeight: 700, color: TX, marginBottom: 8 }}>{f.title}</div>
                <p style={{ fontSize: 11, color: TX2, lineHeight: 1.6, margin: 0 }}>{f.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ background: u.color + '10', border: `1px solid ${u.color}30`, borderRadius: 16, padding: '32px 36px', marginBottom: 72, textAlign: 'center' }}>
          <div style={{ fontSize: 34, color: u.color, marginBottom: 16, opacity: 0.4 }}>"</div>
          <p style={{ fontFamily: 'Sora, system-ui', fontSize: 16, fontWeight: 600, color: TX, lineHeight: 1.6, marginBottom: 20, fontStyle: 'italic', maxWidth: 600, margin: '0 auto 20px' }}>
            {u.testimonialQuote}
          </p>
          <div style={{ fontSize: 11, fontWeight: 700, color: TX }}>{u.testimonialName}</div>
          <div style={{ fontSize: 10, color: TX3, marginTop: 3 }}>{u.testimonialRole}</div>
        </div>
        ...
        <div style={{ background: TX, borderRadius: 16, padding: '40px 32px', textAlign: 'center', marginBottom: 48 }}>
          ...
          <Link href={localePath('/signin', lang)} style={{ fontSize: 13, fontWeight: 700, color: TX, background: ACC, borderRadius: 10, padding: '14px 32px', textDecoration: 'none', display: 'inline-block' }}>
            {u.cta} →
          </Link>
        </div>
        ...
        {/* Other use cases */}
        <div>
          <h3 style={{ fontFamily: 'Sora, system-ui', fontSize: 13, fontWeight: 700, color: TX, marginBottom: 14 }}>AskBiz also works for</h3>
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            {USE_CASES.filter(x => x.slug !== u.slug).map(x => (
              <Link key={x.slug} href={localePath(`/for/${x.slug}`, lang)} style={{ fontSize: 11, color: TX2, background: SF, border: `1px solid ${BD}`, borderRadius: 8, padding: '6px 14px', textDecoration: 'none' }}>
                {x.icon} {x.title}
              </Link>
            ))}
          </div>
