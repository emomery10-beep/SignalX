...
export const TRANSPARENCY_SECTIONS: TransparencySection[] = [
  {
    ...
    articles: [
      ...
      },
      {
        ...
        readTime: 4,
        lastUpdated: "2026-04-01",
        keywords: ["data sent to ai askbiz", "ai prompt data", "business data ai exposure", "what does askbiz send to anthropic"],
        content: [
          {
          ...
          {
            heading: "Data Handling at Anthropic",
            body: "Data sent to Anthropic via the API is:\n- **Transient** — processed to generate the response and not retained\n- **Not used for training** — Anthropic's API terms prohibit using API calls to train models\n- **Encrypted in transit** — all API communication uses TLS 1.3\n- **Subject to Anthropic's DPA** — we have a data processing agreement with Anthropic covering GDPR obligations\n\nAnthropicis based in San Francisco, California. Data transferred to their API from EU/UK users is governed by Standard Contractual Clauses (SCCs). The transfer is transient — data is not stored at rest in the US.",
          },
        ],
