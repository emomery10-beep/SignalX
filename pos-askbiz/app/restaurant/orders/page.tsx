    ...
    <div className="pos-screen" style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9', fontFamily: 'system-ui, sans-serif', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div style={{ width: 320, borderRight: '1px solid #1e293b', overflow: 'auto' }}>
          {loading && <div style={{ padding: 20, color: '#64748b', fontSize: 13 }}>{tc(NS + 'loading_orders')}</div>}
            ...
            return (
              <button key={order.id} type="button" className="pos-item"
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                  ...
                  <span style={{ background: stColor + '20', color: stColor, borderRadius: 10, padding: '2px 8px', fontSize: 11, fontWeight: 700 }}>{tc(NS + 'status_' + order.status)}</span>
                </div>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <div>
                  ...
                  <div style={{ color: '#64748b', fontSize: 13 }}>{tc(NS + 'covers_elapsed', { covers: selected.covers, elapsed: elapsed(selected.seated_at) })}</div>
                </div>
                  ...
                  {(menu.find(c => c.id === activeCat)?.items || []).map(item => {
                    ...
                    return (
                      <button key={item.id} onClick={() => addToCart(item)} disabled={item.eighty_sixed}
                        ...
                        {item.eighty_sixed && <div style={{ color: '#ef4444', fontSize: 11, marginTop: 2 }}>{tc(NS + 'eighty_sixed')}</div>}
                        {inCart > 0 && (
              ...
              <div style={{ width: 280, background: '#1e293b', borderRadius: 12, padding: 16, height: 'fit-content', position: 'sticky', top: 0 }}>
                ...
                {!selected && (
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>{tc(NS + 'covers_label')}</label>
                    <input type="number" value={covers} min={1} max={20} onChange={e => setCovers(parseInt(e.target.value) || 1)}
                      style={{ ...inp, width: '100%' }} />
                  </div>
                )}
                {cart.length === 0 && <div style={{ color: '#64748b', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>{tc(NS + 'tap_items_to_add')}</div>}
                {cart.map(c => (
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                  <button onClick={() => setView('active')} style={{ flex: 1, background: '#334155', border: 'none', color: '#94a3b8', padding: '10px', borderRadius: 8, cursor: 'pointer' }}>{tc(NS + 'cancel')}</button>
                  <button onClick={sendOrder} disabled={!cart.length || saving} className="pos-btn-primary"
                    ...
                    {saving ? tc(NS + 'sending') : tc(NS + 'send_to_kitchen')}
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="pos-sheet" style={{ maxWidth: 420, margin: '0 auto' }}>
              ...
              <div style={{ background: '#1e293b', borderRadius: 12, padding: 16, marginBottom: 16 }}>
                ...
                <div style={{ marginBottom: 10 }}>
                  <label style={{ fontSize: 11, color: '#64748b', display: 'block', marginBottom: 4 }}>{tc(NS + 'discount_label', { sym })}</label>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setView('active')} style={{ flex: 1, background: '#334155', border: 'none', color: '#94a3b8', padding: '14px', borderRadius: 10, cursor: 'pointer' }}>{tc(NS + 'back')}</button>
