...
export default function IntelligencePage() {
  ...
  useEffect(() => {
    ...
    geoPoints.forEach(p => {
      ...
      const marker = L.circleMarker([p.lat, p.lng], {
        ...
        fillColor:   isSelected ? '#dc2626' : ACC,
        color:       '#fff',
        weight:      2.5,
        fillOpacity: 0.9,
      }).addTo(map)
      ...
      marker.bindPopup(`
        <div style="font-family:-apple-system,sans-serif;min-width:150px;padding:2px 0">
          <div style="font-weight:800;font-size:16px;color:#1a1916">${sym}${p.total.toFixed(2)}</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9f8f6', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* Header */}
      <div style={{ padding: '14px 20px 10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#fff', borderBottom: '1px solid #e5e2dc' }}>
        <div>
          <div style={{ fontWeight: 800, fontSize: 18, color: '#1a1916' }}>Sales Map</div>
          <div style={{ fontSize: 12, color: '#6b6760' }}>Geo-tagged transactions</div>
        </div>
        <button onClick={() => router.back()} style={{ padding: '6px 14px', borderRadius: 8, border: '1px solid #e5e2dc', background: 'transparent', fontSize: 13, cursor: 'pointer', color: '#6b6760' }}>
          ← Back
        </button>
      </div>

      {/* Period tabs */}
      <div style={{ display: 'flex', gap: 6, padding: '10px 20px', background: '#fff', borderBottom: '1px solid #e5e2dc' }}>
        {(['1', '7', '30', '90'] as Period[]).map(p => (
          <button key={p} onClick={() => setPeriod(p)} style={{ padding: '6px 14px', borderRadius: 9999, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: 'none', background: period === p ? ACC : '#f9f8f6', color: period === p ? '#fff' : '#6b6760' }}>
            {periodLabel(p)}
          </button>
        ))}
      </div>
      ...
      {/* Stats */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', background: '#fff', borderBottom: '1px solid #e5e2dc' }}>
        {[
          ...
          <div key={i} style={{ padding: '10px 16px', borderRight: i < 2 ? '1px solid #e5e2dc' : 'none' }}>
            <div style={{ fontSize: 10, color: '#a39e97', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 2 }}>{s.label}</div>
            <div style={{ fontSize: 17, fontWeight: 800, color: '#1a1916' }}>{s.value}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ position: 'relative', height: 320, flexShrink: 0, background: '#d4e2d1' }}>
        ...
        {!loading && geoPoints.length === 0 && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(249,248,246,0.93)', gap: 8, zIndex: 999 }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 700, color: '#1a1916' }}>No location data yet</div>
            <div style={{ fontSize: 13, color: '#6b6760', textAlign: 'center', maxWidth: 260, lineHeight: 1.5 }}>
        ...
        {loading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(249,248,246,0.8)', zIndex: 999 }}>
            <div style={{ fontSize: 13, color: '#6b6760' }}>Loading…</div>
          </div>
        )}
      </div>
      ...
      {/* Selected sale */}
      {selected && (
        <div style={{ margin: '10px 20px 0', padding: '12px 16px', borderRadius: 14, background: 'rgba(208,138,89,.07)', border: '1px solid rgba(208,138,89,.3)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ fontSize: 12, color: '#6b6760', marginBottom: 2 }}>
          ...
          <button onClick={() => setSelected(null)} style={{ background: 'none', border: 'none', fontSize: 22, cursor: 'pointer', color: '#a39e97', lineHeight: 1 }}>×</button>
        </div>
      )}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '10px 20px 40px' }}>
        {geoPoints.length > 0 && (
          <div style={{ fontSize: 11, fontWeight: 700, color: '#a39e97', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 8 }}>
            {geoPoints.length} geo-tagged sale{geoPoints.length !== 1 ? 's' : ''}
          </div>
        )}

        {geoPoints.slice().reverse().map(p => (
          <div key={p.id} onClick={() => flyTo(p)}
            style={{ background: '#fff', borderRadius: 12, border: `1.5px solid ${selected?.id === p.id ? ACC : '#e5e2dc'}`, marginBottom: 8, padding: '12px 16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: 15, fontWeight: 700, color: '#1a1916' }}>{sym}{p.total.toFixed(2)} {payIcon(p.payment_type)}</div>
              ...
              {p.cleanNotes && <div style={{ fontSize: 11, color: '#a39e97', marginTop: 1 }}>{p.cleanNotes}</div>}
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0, marginLeft: 12 }}>
              <div style={{ width: 10, height: 10, borderRadius: '50%', background: selected?.id === p.id ? '#dc2626' : ACC }} />
              <div style={{ fontSize: 10, color: '#a39e97' }}>{p.lat.toFixed(3)}, {p.lng.toFixed(3)}</div>
            </div>
          </div>
        ))}
        ...
        {!loading && geoPoints.length === 0 && allCount > 0 && (
          <div style={{ padding: '32px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 600, color: '#1a1916', marginBottom: 4 }}>{allCount} sales — none geo-tagged yet</div>
            <div style={{ fontSize: 13, color: '#6b6760' }}>The cashier device must allow location access before checking out.</div>
          </div>
        )}

        {!loading && allCount === 0 && (
          <div style={{ padding: '32px 0', textAlign: 'center', color: '#6b6760', fontSize: 14 }}>No sales in this period yet.</div>
        )}
      </div>
    </div>
  )
