...
function aggregateByDate(
  ...
  if (!dateCol) {
    ...
    return rows.map(r => Number(r[valueCol])).filter(v => !isNaN(v) && isFinite(v))
  }
...
export function forecastFromDataset(
  ...
  // Check if targetColumn contains numeric data
  const rawValues = rows.map(r => Number(r[targetColumn])).filter(v => !isNaN(v) && isFinite(v))

  if (rawValues.length < 3) {
    // targetColumn is not numeric — find the best numeric column to forecast instead
    const numericCols = detectNumericColumns(rows)
    
    if (numericCols.length === 0) {
      throw new Error(`No numeric columns found in your data. Forecasting requires columns with numbers (sales, revenue, units, etc.). Your dataset has: ${Object.keys(rows[0]).join(', ')}`)
    }

    // Pick the best candidate: prefer sales/revenue/units columns
    ...
    const values = aggregateByDate(rows, preferred, dateCol)

    if (values.length < 3) {
      throw new Error(`Column "${preferred}" has fewer than 3 data points after aggregation. Try uploading more historical data.`)
    }

    const result = forecast(values, horizonPoints, method)
    return {
      ...result,
      summary: `Note: "${targetColumn}" contains text (product names, categories, etc.) and cannot be forecast directly. AskBiz automatically forecast your "${preferred}" column instead — ${result.summary}`,
    }
  }

  // Numeric column — aggregate by date if possible
  const dateCol = detectDateColumn(rows)
  const values = dateCol ? aggregateByDate(rows, targetColumn, dateCol) : rawValues

  if (values.length < 3) {
    throw new Error(`Need at least 3 data points to forecast. Found ${values.length} in "${targetColumn}". Try uploading more rows.`)
  }
