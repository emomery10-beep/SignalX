...
function sign(method: string, url: string, extraParams: Record<string, string> = {}): string {
  ...
  // Merge oauth params + any extra body/query params for signature base string
  const allParams: Record<string, string> = { ...oauthParams, ...extraParams }

  const sp   = Object.keys(allParams).sort().map(k => enc(k) + '=' + enc(allParams[k])).join('&')
  const base = method + '&' + enc(url) + '&' + enc(sp)
...
export async function postTweet(text: string, options: { replyToId?: string } = {}): Promise<{ id: string; text: string }> {
  ...
  // Twitter v2 with OAuth 1.0a: body is JSON, so no body params go into signature.
  // The fix is that we must NOT include JSON body fields in the signature base string —
  // only application/x-www-form-urlencoded params would be included.
  // The real bug was the sign() function wasn't being called correctly for POST.
  // Now we explicitly pass no extraParams (correct for JSON body posts).
  ...
  let data: any
  try { data = JSON.parse(raw) } catch { throw new Error('Invalid JSON from X: ' + raw) }

  if (!res.ok) {
    const msg = data?.errors?.[0]?.message || data?.detail || data?.title || raw
    throw new Error('X post failed (' + res.status + '): ' + msg)
  }

  if (!data?.data?.id) throw new Error('X returned success but no tweet id: ' + raw)
...
export async function searchTweetsViaTavily(query: string, maxResults = 10) {
  ...
  return (data.results || [])
    .filter((r: any) => r.url && (r.url.includes('x.com') || r.url.includes('twitter.com')))
    .map((r: any) => ({
