...
function sign(method: string, url: string): string {
  ...
  const p: Record<string,string> = { oauth_consumer_key:ck, oauth_nonce:nonce, oauth_signature_method:'HMAC-SHA1', oauth_timestamp:ts, oauth_token:at, oauth_version:'1.0' }
  const enc = (s: string) => encodeURIComponent(s)
  const sp = Object.keys(p).sort().map(k => enc(k)+'='+enc(p[k])).join('&')
...
export async function postTweet(text: string, options: { replyToId?: string } = {}): Promise<{ id: string; text: string }> {
  ...
  const res = await fetch(url, { method:'POST', headers:{ Authorization:sign('POST',url), 'Content-Type':'application/json' }, body:JSON.stringify(body) })
...
export async function searchTweetsViaTavily(query: string, maxResults = 10) {
  ...
  const res = await fetch('https://api.tavily.com/search', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ api_key:key, query:'site:x.com OR site:twitter.com '+query, search_depth:'basic', max_results:maxResults, include_answer:false }) })
  if (!res.ok) throw new Error('Tavily search failed: '+res.status)
  const data = await res.json()
  return (data.results||[]).filter(r => r.url&&(r.url.includes('x.com')||r.url.includes('twitter.com'))).map(r => ({
