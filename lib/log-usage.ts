...
function costUsd(model: string, inputTokens: number, outputTokens: number): number {
  ...
  // Claude tier fallback — default to haiku (cheapest) rather than sonnet,
  // since unmatched models here are far more likely to be a new cheap Groq
  // model than an actual Sonnet call (those are logged with an exact match).
