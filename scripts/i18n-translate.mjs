...
async function translate(lang, langName, ns, entries) {
...
Rules:
- Return ONLY a JSON object with the SAME keys; translate the values.
...
async function run() {
  ...
  for (const ns of namespaces) {
    ...
    for (const [lang, langName] of Object.entries(TARGETS)) {
      ...
      process.stdout.write(`  ${lang}/${ns}: translating ${Object.keys(missing).length} key(s)… `)
      const translated = await translate(lang, langName, ns, missing)

      // Rebuild in en key order, preferring existing reviewed values over new ones.
      const merged = {}
      for (const k of Object.keys(enDict)) merged[k] = (FORCE ? translated[k] : (existing[k] ?? translated[k])) ?? enDict[k]
