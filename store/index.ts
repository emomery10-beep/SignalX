...
interface SessionState {
  ...
  pulseSignal: { text: string; query: string; level: 'good' | 'warning' | 'risk' } | null
}
...
interface SignalXStore {
  ...
  setPulseSignal: (p: SessionState['pulseSignal']) => void
  toggleCfoMode: () => void
  reset: () => void
}
...
export const useStore = create<SignalXStore>()(
  persist(
    (set) => ({
      user: { id: null, name: '', email: '', initials: '', plan: 'free' },
      geo: null,
      ...
      setSimulateMode: (v) => set(s => ({ session: { ...s.session, simulateMode: v } })),
      setPulseSignal: (p) => set(s => ({ session: { ...s.session, pulseSignal: p } })),
      toggleCfoMode: () => set(s => ({ settings: { ...s.settings, cfoMode: !s.settings.cfoMode } })),
      reset: () => set({ user: { id: null, name: '', email: '', initials: '', plan: 'free' }, session: defaultSession }),
    }),
    {
      name: 'signalx-store',
